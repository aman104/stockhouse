...
function sp_woo_before_main_content() {
	...
	$output .= '<h1 class="page-title">' . ob_get_clean() . '</h1>' . "\r\n";
	...
	$output .= '</header><!--close .page-header -->' . "\r\n";

	ob_start();
	do_action( 'sp_woo_before_main_content_below_header' );
	$output .= ob_get_clean();

	$output .= '<div class="container main-container woocommerce-container" itemtype="http://schema.org/Product" itemscope>' . "\r\n";
	...
	$output .= '<section id="primary" role="main" class="' . esc_attr( $span_columns ) . '">' . "\r\n";
...
function sp_woo_after_main_content() {
	...
	$output .= '</div><!--close .row--></div><!--close .woocommerce-container-->' . "\r\n";
 ...
 */
function sp_woo_product_add_to_cart_text() {
	return '<i class="icon-plus" aria-hidden="true"></i> ' . __( 'Add to Cart', 'sp-theme' );
}
endif;
...
function sp_woo_product_loop_add_to_cart_link( $html, $product ) {
	...
	if ( $product->product_type === 'variable' || $product->product_type === 'grouped' ) {
		...
		$output .= '<a href="' . esc_url( $product->add_to_cart_url() ) . '" rel="nofollow" data-product_id="' . esc_attr( $product->id ) . '" data-product_sku="' . esc_attr( $product->get_sku() ) . '" class="button product_type_' . esc_attr( $product->product_type ) . '"><i class="icon-plus" aria-hidden="true"></i> ' . $product->add_to_cart_text() . '</a>' . "\r\n";
	} else {

		$output .= '<a href="' . esc_url( $product->add_to_cart_url() ) . '" rel="nofollow" data-product_id="' . esc_attr( $product->id ) . '" data-product_sku="' . esc_attr( $product->get_sku() ) . '" class="add_to_cart_button button product_type_' . esc_attr( $product->product_type ) . '"><i class="icon-plus" aria-hidden="true"></i> ' . $product->add_to_cart_text() . '</a>' . "\r\n";
	}
...
function sp_woo_single_product_action_meta() {
	...
	$output .= '<div class="action-meta clearfix">' . "\r\n";
...
function sp_woo_add_product_wishlist_to_my_account() {
	...
	$output .= '<h3 class="title-with-line"><span>' . __( 'My Wishlists', 'sp-theme' ) . '</span></h3>' . "\r\n";

	// check any saved wishlists
	if ( isset( $saved_wishlists ) && is_array( $saved_wishlists ) && count( $saved_wishlists ) > 0 ) {
		$output .= '<ol class="my-account-wishlists">' . "\r\n";
		...
		foreach( $saved_wishlists as $list_name => $list_val ) {
			...
			$output .= '<li><a href="' . esc_url( $url ) . '" title="' . esc_attr( stripslashes( $list_name ) ) . '">' . stripslashes( $list_name ) . '</a><a href="#" title="' . esc_attr__( 'Delete Wishlist Entry', 'sp-theme' ) . '" class="delete-wishlist-entry" data-wishlist-name="' . esc_attr( stripslashes( $list_name ) ) . '"><i class="icon-remove-sign" aria-hidden="true"></i></a></li>' . "\r\n";
		}
...
function sp_woo_checkout_breadcrumb( $checkout = '', $active = 'signin' ) {
	...
	if ( sp_get_option( 'show_checkout_breadcrumbs', 'is', 'off' ) )
		return;
	...
	$output .= apply_filters( 'sp_woo_checkout_breadcrumb_signin', '<li><a href="#" title="' . esc_attr__( '1. Sign In / Register', 'sp-theme' ) . '" class="' . esc_attr( $signin_active ) . ' signin-bc-link">' . esc_attr__( '1. Sign In / Register', 'sp-theme' ) . '</a></li>' ) . "\r\n";

	$output .= apply_filters( 'sp_woo_checkout_breadcrumb_billing_shipping', '<li><a href="#" title="' . esc_attr__( '2. Billing / Shipping', 'sp-theme' ) . '" class="' . esc_attr( $billing_active ) . ' billing-bc-link">' . esc_attr__( '2. Billing / Shipping', 'sp-theme' ) . '</a></li>' ) . "\r\n";

	$output .= apply_filters( 'sp_woo_checkout_breadcrumb_review_order', '<li><a href="#" title="' . esc_attr__( '3. Review / Payment', 'sp-theme' ) . '" class="' . esc_attr( $review_active ) . ' review-bc-link">' . esc_attr__( '3. Review / Payment', 'sp-theme' ) . '</a></li>' ) . "\r\n";

	$output .= apply_filters( 'sp_woo_checkout_breadcrumb_confirmation', '<li class="confirmation"><a href="#" title="' . esc_attr__( '4. Confirmation', 'sp-theme' ) . '" class="' . esc_attr( $confirm_active ) . ' confirmation-bc-link">' . esc_attr__( '4. Confirmation', 'sp-theme' ) . '</a></li>' ) . "\r\n";
