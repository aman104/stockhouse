...
function sp_display_page_title() {
	...
	if ( ! isset( $show_title ) || $show_title === 'on' ) {
		...
		if ( isset( $show_tagline ) && $show_tagline === 'on' && isset( $tagline ) && ! empty( $tagline ) ) { 
			echo '<h1 class="entry-title">' . get_the_title( $post->ID ) . ' - <span class="heading-tagline">' . $tagline . '</span></h1>' . "\r\n";
		} else {
			echo '<h1 class="entry-title"><span>' . get_the_title( $post->ID ) . '</span></h1>' . "\r\n";
		}
	}
...
function sp_display_header_meta_icons() {
	...
	if ( sp_woo_exists() ) {
		...
		if ( sp_get_option( 'show_mini_cart', 'is', 'on' ) ) {
		    ...
		    $output .= '<div id="mini-cart">' . "\r\n";
		    $output .= '<i class="icon-cart cart-icon-button" aria-hidden="true"><span class="product-item-count">' . $product_item_count . '</span></i>' . "\r\n";

		    ob_start();
		    sp_display_mini_cart();
		    $output .= ob_get_clean();

		    $output .= '</div><!--close #mini-cart-->' . "\r\n";
		}

		// check if show wishlist icon is on
		if ( sp_get_option( 'show_wishlist_icon', 'is', 'on' ) )
			$output .= '<a href="' . get_permalink( sp_wishlist_page_id() ) . '" title="' . esc_attr__( 'My Wishlist', 'sp-theme' ) . '" class="get-wishlist sp-tooltip" data-toggle="tooltip" data-placement="top"><i class="icon-star" aria-hidden="true"><span class="wishlist-item-count">' . sp_woo_get_wishlist_item_count() . '</span></i></a>' . "\r\n";

		// check if show compare icon is on
		if ( sp_get_option( 'show_compare_icon', 'is', 'on' ) )
			$output .= '<a href="' . get_permalink( sp_compare_page_id() ) . '" title="' . esc_attr__( 'Compare Products', 'sp-theme') . '" class="get-compare sp-tooltip"><i class="icon-cog" aria-hidden="true"><span class="compare-item-count" data-toggle="tooltip" data-placement="top">' . sp_woo_get_compare_item_count() . '</span></i></a>' . "\r\n";
	}

	if ( sp_get_option( 'display_search', 'is', 'on' ) )
		$output .= '<a href="#" title="' . esc_attr__( 'Search', 'sp-theme') . '" class="search-icon-button sp-tooltip" data-toggle="tooltip" data-placement="top"><i class="icon-search" aria-hidden="true"></i></a>' . "\r\n";
...
function sp_show_post_content_excerpt() {
	...
	if ( strlen( $post->post_excerpt ) > 0 ) {
		...
		if ( apply_filters( 'sp_more_link_off', true ) )
			the_content( '', false, '' );
		else
