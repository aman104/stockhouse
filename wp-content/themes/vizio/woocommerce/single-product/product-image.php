...
if ( has_post_thumbnail() ) {
	if ( is_product() ) {
		...
		$image_size = sp_get_image( get_post_thumbnail_id( $product->id ), $image_size['width'], $image_size['height'], (bool)$image_size['crop'] );

		$image_size = sp_get_image_size( $image_size['url'] );
	} else {
		...
		$image_size = sp_get_image( get_post_thumbnail_id( $product->id ), $image_width, $image_height, $image_crop );

		$image_size = sp_get_image_size( $image_size['url'] );
	}

	$image_size = $image_size['width'] . 'px';
} else {
	$image_size = 'auto';
}
?>
<div class="images <?php echo sp_column_css( '', '', '', '5' ); ?>">
	...
	<div class="image-wrap" style="max-width:<?php echo esc_attr( $image_size ); ?>;">
	<?php
		if ( has_post_thumbnail() ) {
