...
jQuery( document ).ready( function( $ ) {
	...
	$.sp_theme = {
		...
		woocommerce: function() {
			...
			if ( $( '.product .image-wrap' ).hasClass( 'quickview' ) ) {
				...
				$( '.product .image-wrap' ).on( 'click', '.product-image-link .quickview-button', function( e ) {
					...
					$.post( sp_theme.ajaxurl, $data, function( response ) {
						...
						// add the plus minus quantity buttons
						...
						// product image zoom
						if ( sp_theme.product_image_zoom ) {
						...
						$( '.product' ).on( 'click', 'a.zoom.image-swap', function( e ) {
							...
							parentContainer.find( 'a.woocommerce-main-image, a.product-image-link' ).append( spinner.el ).find( '.spinner' ).center();
						...
						case 'variable' :
							...
							$.post( sp_theme.ajaxurl, $data, function( response ) {
								...
								if ( response.redirect ) {
									...
									// check if interactive add to cart is on
									if ( sp_theme.show_interactive_add_to_cart === 'on' ) {
			...
			/////////////////////////////////////////////////////
			// set view type on product category on load
			/////////////////////////////////////////////////////					
			var viewType = $.cookie( 'sp_view_type' );

			// if cookie exists and is list view
			if ( viewType !== 'undefined' && viewType === 'list-view' ) {
				$( '.post-type-archive-product ul.products, .tax-product_cat ul.products' ).each( function() {
					$( this ).addClass( 'list-view' );

					// clear after each product item
					$( 'li', this ).addClass( 'clearfix' );

					// hide the original
					$( 'li', this ).find( '.image-wrap .action-meta' ).css( 'visibility', 'hidden' );
			...
			$( '.single-product .product' ).each( function() {
				// get width of image thumbnail
				var thumbWidth = $( 'input[name="product_thumb_width"]', this ).val(),
				...
				if ( sp_theme.product_gallery_swap ) {
					$( this ).on( 'click', 'a.image-swap', function( e ) {
						...
						var spinner = new Spinner( opts ).spin();
						imageLink.append( spinner.el ).find( '.spinner' ).center();
			...
			$( 'a.goto-review' ).click( function() {
				...
				if ( $( '.billing-form-section #shiptobilling-checkbox' ).prop( 'checked' ) === false ) {
				...
				$( 'form', container ).submit( function() {
					...
					$.post( sp_theme.ajaxurl, $data, function( response ) {
						...
						if ( typeof response.error_msg === 'object' ) {
							...
							// phone
							if ( typeof response.error_msg.phone !== 'undefined' && response.error_msg.phone.length ) {
								container.find( 'input[name=phone]' ).parents( '.form-group' ).addClass( 'error' ).find( 'p.alert span' ).html( response.error_msg.phone ).parent().addClass( 'alert-danger' ).show();
							}
							// regulamin
							if ( typeof response.error_msg.regulamin !== 'undefined' && response.error_msg.regulamin.length ) {
								container.find( 'input[name=regulamin]' ).parents( '.form-group' ).addClass( 'error' ).find( 'p.alert span' ).html( response.error_msg.regulamin ).parent().addClass( 'alert-danger' ).show();
							}
							// tos
							if ( typeof response.error_msg.tos !== 'undefined' && response.error_msg.tos.length ) {
								container.find( 'input[name=tos]' ).parents( '.form-group' ).addClass( 'error' ).find( 'p.alert span' ).html( response.error_msg.tos ).parent().addClass( 'alert-danger' ).show();
							}
							// captcha
