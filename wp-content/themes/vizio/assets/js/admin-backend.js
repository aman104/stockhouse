...
jQuery( document ).ready( function( $ ) {
	...
	$.SPFrameWork = {	
		...
		closePopup: function( moduleElement, action ) {
			...
			try {
				newContent = window.tinyMCE.get( 'pb_tinymce' ).getContent();

				// remove tinymce instance
				window.tinyMCE.execCommand( 'mceRemoveControl', true, 'pb_tinymce' );				
			}
			catch ( err ) {
				newContent = switchEditors.wpautop( $( 'textarea.sp-tinymce-content' ).val() );
			}
		...
		init: function() {
			...
			function contactFormAddOption() { 
				...
				$( 'ul.add-option-row' ).on( 'click', '.add-option', function( e ) { 
					...
					var addButton = $( this ),
						...
						optionRow = $( '<ul class="option-row"><li><i class="indent" aria-hidden="true"></i> ' + sp_theme.contact_form_option_name + ' <input type="text" name="field_options[][field_' + fieldID + '][]" value="" /> <a href="#" class="remove-option button" title="' + sp_theme.contact_form_remove_option + '">' + sp_theme.contact_form_remove_option + '</a><i class="icon-reorder option-field-handle" aria-hidden="true"></i></li></ul>' );	
				...
				select.on( 'change', function() {
					...
					switch( selected ) {
						...
						case 'multiselect' :
							...
							var optionRow = $( '<ul class="option-row"><li><i class="indent" aria-hidden="true"></i> ' + sp_theme.contact_form_option_name + ' <input type="text" name="field_options[][field_' + fieldID + '][]" value="" /> <a href="#" class="remove-option button" title="' + sp_theme.contact_form_remove_option + '">' + sp_theme.contact_form_remove_option + '</a><i class="handle" aria-hidden="true"></i></li></ul><ul class="add-option-row"><li><i class="indent" aria-hidden="true"></i> <a href="#" class="add-option button" title="' + sp_theme.contact_form_add_option + '">' + sp_theme.contact_form_add_option + '</a><a href="#" class="toggle-options button" title="' + sp_theme.contact_form_toggle_options + '">' + sp_theme.contact_form_toggle_options + '</a></li></ul>' );	
			...
			/////////////////////////////////////////////////////
			$( '.page-builder-section' ).on( 'click', 'a.one, a.two, a.three, a.four, a.five, a.six, a.two-third, a.one-third', function( e ) {
				// prevent default behavior
				e.preventDefault();
				...
				var rowInner = $( this ).parents( '.row-inner' ),
					...
					columnHTML = $( '<div class="column"><h3>' + sp_theme.page_builder_column_heading_text + '</h3><div class="inner"><div class="module-element column-placeholder"><div class="module-values"><input type="hidden" name="pb[row][][column][][module][0][placeholder]" class="module-value" value="hold" /></div></div></div></div>' ),
					columnType = $( this ).prop( 'class' ),
					modules = '',
					columnCount,
					columnNumber;
