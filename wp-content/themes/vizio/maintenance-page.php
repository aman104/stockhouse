...
if ( wp_verify_nonce( $nonce, 'sp_maintenance_submit_email' ) && isset( $_POST ) && isset( $_POST['submit_form'] ) && isset( $_POST['email'] ) && ! empty( $_POST['email'] )  ) {
	...
	$to = $admin_email;
	$subject = get_bloginfo( 'name' ) . ' ' . apply_filters( 'sp_maintenance_email_notification_subject', __( '- Maintenance Email Notification Request', 'sp-theme' ) );
	$headers = sprintf( __( '%s', 'sp-theme' ) . ' ' . get_bloginfo( 'name' ) . ' <' . $email . '>', 'From:' ) . "\r\n";
