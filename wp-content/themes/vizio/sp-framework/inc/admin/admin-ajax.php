...
function sp_font_select_ajax() {
	...
	if ( $font ) {
		...
		$variants .= '<option value="none">' . __( '--Please Select--', 'sp-theme' ) . '</option>' . "\r\n";
		$subsets .= '<option value="none">' . __( '--Please Select--', 'sp-theme' ) . '</option>' . "\r\n";

		foreach( $font['variants'] as $variant ) {
			$variants .= '<option value="' . $variant . '">' . $variant . '</option>' . "\r\n";
		}
		...
		foreach( $sptheme_config['init']['customizer'] as $setting ) {				
			if ( $setting['_attr']['id'] === $setting_name && $setting_value !== $setting['_attr']['std'] && $setting['_attr']['selector'] !== 'none' && $setting['_attr']['property'] !== 'none' ) {
				...
				if ( $setting['_attr']['property'] === 'font-family' && isset( $setting_value ) && ! empty( $setting_value ) && ! sp_check_default_fonts( $setting_value ) ) {
					...
					$styles .= sp_generate_css( $setting['_attr']['selector'], $setting['_attr']['property'], $setting_value, $setting['_attr']['prefix'], $setting['_attr']['suffix'] ) . "\r\n";
				}
			}
		}
	}
...
function sp_page_builder_content_tinymce_ajax() {
	...
	// content
	ob_start();
	wp_editor( $content, 'pb_tinymce', $settings );
	$output = ob_get_clean();
...
function sp_add_custom_product_tab_ajax() {
	...
	$tab = '<li class="tab"><a href="" class="tab-title">' . __( 'Untitled', 'sp-theme' ) . '</a><span class="edit-title"><input type="hidden" name="sp_product_tab_name[]" value="' . __( 'Untitled', 'sp-theme' ) . '" placeholder="' . esc_attr__( 'Enter Tab Name', 'sp-theme' ) . '" class="tab-name" /></span><a href="#" class="remove-tab" title="' . esc_attr__( 'Remove Tab', 'sp-theme' ) . '">&times;</a></li>' . "\r\n";

	$editor = '<div id="" class="tab-content">' . "\r\n";
