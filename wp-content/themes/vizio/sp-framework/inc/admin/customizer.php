...
function _sp_customize_register( $wp_customize ) { 
	...
	class Customize_Text_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$output .= '</label>' . "\r\n";
			$output .= '<input type="text" ' . $this->get_link() . ' value="' . esc_attr( $this->value() ) . '" />' . "\r\n";
			$output .= '<input type="hidden" data-default-setting="' . esc_attr( $this_default ) . '" class="default-setting" />' . "\r\n";
			$output .= '</div>' . "\r\n";
	...
	class Customize_Text_Size_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$output .= '<a href="#" title="' . esc_attr__( 'Default', 'sp-theme' ) . '" data-default="' . esc_attr( $this_default ) . '" class="default-size button button-small">' . __( 'Default', 'sp-theme' ) . '</a>' . "\r\n";
			$output .= '<input type="hidden" data-default-setting="' . esc_attr( $this_default ) . '" class="default-setting" />' . "\r\n";
			$output .= '</div>' . "\r\n";
	...
	class Customize_Font_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$output .= '<span class="customize-control-title">' . esc_html( $this->label ) . '</span>' . "\r\n";
			$output .= '</label>' . "\r\n";
			$output .= '<select ' . $this->get_link() . ' class="select2-select font-select" data-no_results_text="' . __( 'No results match', 'sp-theme' ) . '">' . "\r\n";
			$output .= '<option value="none">' . __( '--Please Select--', 'sp-theme' ) . '</option>' . "\r\n";

			foreach( $fonts as $font ) {
				$output .= '<option value="' . esc_attr( $font['family'] ) . '" ' . selected( $this_default, $font['family'], false ) . '>' . $font['family'] . '</option>' . "\r\n";
			}
	...
	class Customize_Font_Variant_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$output .= '<span class="customize-control-title">' . esc_html( $this->label ) . '</span>' . "\r\n";
			$output .= '</label>' . "\r\n";
			$output .= '<select ' . $this->get_link() . ' class="select2-select font-variant-select">' . "\r\n";
			$output .= '<option value="none">' . __( '--Please Select--', 'sp-theme' ) . '</option>' . "\r\n";

			if ( is_array( $variants ) && isset( $variants ) ) {
				foreach( $variants['variants'] as $variant ) {
					$output .= '<option value="' . esc_attr( $variant ) . '" ' . selected( $this_default, $variant, false ) . '>' . $variant . '</option>' . "\r\n";
				}
			}
	...
	class Customize_Font_Subset_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$output .= '<span class="customize-control-title">' . esc_html( $this->label ) . '</span>' . "\r\n";
			$output .= '</label>' . "\r\n";
			$output .= '<select ' . $this->get_link() . ' class="select2-select font-subset-select" multiple="multiple" data-placeholder="' . __( '--Please Select--', 'sp-theme' ) . '">' . "\r\n";
			$output .= '<option value="none">' . __( '--Please Select--', 'sp-theme' ) . '</option>' . "\r\n";

			if ( is_array( $subsets ) && isset( $subsets ) ) {
				foreach( $subsets['subsets'] as $subset ) {
			   		$output .= '<option value="' . esc_attr( $subset ) . '" ' . selected( $this_default, $subset, false ) . '>' . $subset . '</option>' . "\r\n";
				}
			}
	...
	class Customize_Select_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$output .= '<label>' . "\r\n";
			$output .= '<span class="customize-control-title">' . esc_html( $this->label ) . '</span>' . "\r\n";
			$output .= '<select ' . $this->get_link() . ' class="select2-select">' . "\r\n";
			foreach ( $this->choices as $value => $label )
				$output .= '<option value="' . esc_attr( $value ) . '"' . selected( $this->value(), $value, false ) . '>' . $label . '</option>' . "\r\n";
	...
	class Customize_BG_Predefined_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$output .= '<span class="customize-control-title">' . esc_html( $this->label ) . '</span>' . "\r\n";
			$output .= '</label>' . "\r\n";
			$output .= '<select ' . $this->get_link() . ' class="select2-select predefined-bg">' . "\r\n";

			foreach( $options as $k => $v ) {
				$output .= '<option value="' . esc_attr( $k ) . '" ' . selected( $k, $this_default, false ) . '>' . str_replace( array( '.jpg', '.png' ), '', $v ) . '</option>' . "\r\n";
			}
	...
	class Customize_Radio_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			foreach ( $this->choices as $value => $label ) {
				$output .= '<label>' . "\r\n";
				$output .= '<input type="radio" value="' . esc_attr( $value ) . '" name="' . esc_attr( $name ) . '" ' . $this->get_link() . ' ' . checked( $this->value(), $value, false ) . ' />' . "\r\n";
				$output .= esc_html( $label ) . '<br/>' . "\r\n";
				$output .= '</label>' . "\r\n";
			}
	...
	class Customize_Reset_Default_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$output = '';
			$output .= '<div class="control-wrap">' . "\r\n";
			$output .= '<a href="#" title="' . __( 'Reset Section to default settings', 'sp-theme' ) . '" class="customizer-reset-default button">' . __( 'Reset Default Settings', 'sp-theme' ) . '</a>' . "\r\n";
			$output .= '</div>' . "\r\n";
	...
	class Customize_Divider_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$output = '';
			$output .= '<div class="control-wrap">' . "\r\n";
			$output .= '<hr class="customize-divider" />' . "\r\n";
			$output .= '</div>' . "\r\n";
