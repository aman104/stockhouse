...
function sp_admin_menu() {
		...
		add_menu_page( 'sp', THEME_NAME, 'manage_options', 'sp', 'sp_panel', apply_filters( 'sp_backend_panel_menu_icon', THEME_URL . 'images/admin/favicon.png' ), '56.777');
		...
		<form method="post" enctype="multipart/form-data" id="sp-panel-form">	
            ...
            <div id="sp-main-tabs">
            	<div id="header" class="clearfix">
                    ...
                    <div class="bottom clearfix">
                    	...
                    	'<div class="version-meta">' . "\r\n"
                    		. '<small>Theme Version: ' . THEME_VERSION . '</small>' . "\r\n"
                    		. '<small>Framework Version: ' . FRAMEWORK_VERSION . '</small>' . "\r\n"
                ...
                <div id="general" class="panel sp-sub-tabs clearfix">
                    ...
                    <?php
					foreach ( $sptheme_config['tab']['general']['panel'] as $panel ) {
						echo '<div id="general-' . sp_a_clean( $panel['_attr']['title'] ) . '" class="option accordion">' . "\r\n";
							...
							foreach ( $panel['wrapper'] as $wrapper ) {
								...
								if ( $context === 'woo' && ! sp_woo_exists() ) {
									...
									if ( isset( $wrapper['_attr']['title'] ) ) {
										echo '<h3 class="header button-primary button"><span class="caret-closed"></span>' . $wrapper['_attr']['title'] . '</h3>' . "\r\n";
										echo '<div class="accordion-container"><table>' . "\r\n";
										echo '<tr class="layout"><td class="col1"></td><td class="col2"></td><td class="col3"></td><td></td><td></td><td></td></tr>' . "\r\n";
									}
						...
						if ( isset( $panel['wrapper'] ) ) {
							if ( is_array( $panel['wrapper'] ) ) {							
								foreach ( $panel['wrapper'] as $wrapper ) {
									...
									if ( isset( $wrapper['_attr']['title'] ) ) {
										echo '</table>' . "\r\n";
										echo apply_filters( 'sp-control-panel-support-link', '<a href="http://splashingpixels.com/member-dashboard/" title="Splashing Pixels" target="_blank">' . __( 'For more support, please go to our site support forums.', 'sp-theme' ) . '</a>' );
										echo '</div><!--close .accordion-container-->' . "\r\n";
									}							
								} // end 2nd foreach loop
							} 
