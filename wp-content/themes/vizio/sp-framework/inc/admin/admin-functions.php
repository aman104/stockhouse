			...
			foreach( $versions['versions']['item'] as $item ) {
				...
				if ( $item['_attr']['name'] == strtolower( trim( $theme->Name ) ) ) {
					...
					if ( $theme->Version < $cur ) { 
						...
						$notification = '<div class="updated sp-update-notice"><p>' . sprintf( __( 'There is a new version of your theme available (' . '%s' . '). Please update when possible.', 'sp-theme' ), $cur ) . '<a href="' . esc_url( 'http://splashingpixels.com/wp-content/themes/sp/versions/' . strtolower( trim( THEME_NAME ) ) . '_changelog.txt' ) . '" title="' . esc_attr( 'Theme Changelog', 'sp-theme' ) . '" target="_blank">(ChangeLog)</a> (' . __( 'To disable this message, you can turn off notification in your theme options.', 'sp-theme' ) . ')</p></div>';
...
function _sp_get_page_builder_module( $post_id = '', $module = '', $module_settings = '' ) {
	...
	switch( $module ) {
		case 'shortcode' :
			...
			$output = '<div class="module-element" data-id="shortcode">' . "\r\n";
			$output .= '<span class="module-element-name">' . __( 'Shortcode', 'sp-theme' ) . '</span>' . "\r\n";
			$output .= '<a href="#" title="' . __( 'Edit', 'sp-theme' ) . '" class="module-element-edit"><i class="icon-edit" aria-hidden="true"></i></a>' . "\r\n";
			$output .= '<a href="#" title="' . __( 'Delete', 'sp-theme' ) . '" class="module-element-delete"><i class="icon-remove" aria-hidden="true"></i></a>' . "\r\n";
			
			// values
			$output .= '<div class="module-values">' . "\r\n";
			$output .= '<h3>' . $shortcode_name . '</h3>' . "\r\n";
			$output .= '<p class="howto">' . __( 'You may enter any valid shortcode in the box.', 'sp-theme' ) . '</p>' . "\r\n";

			// shortcode
			$output .= '<p><label><strong>' . __( 'Shortcode', 'sp-theme' ) . '</strong></label>' . "\r\n";
			$output .= '<input type="text" name="pb[row][][column][][module][][shortcode][shortcode]" value="' . esc_attr( stripslashes( $shortcode ) ) . '" class="content-area module-value widefat" /></p>' . "\r\n";

			$output .= '</div><!--close .module-values-->' . "\r\n";
			$output .= '</div><!--close .module-element-->' . "\r\n";
			break;	
		...
		case 'custom_content' :
			...
			$output = '<div class="module-element" data-id="custom_content">' . "\r\n";
			$output .= '<span class="module-element-name">' . __( 'Custom Content', 'sp-theme' ) . '</span>' . "\r\n";
			$output .= '<a href="#" title="' . __( 'Edit', 'sp-theme' ) . '" class="module-element-edit"><i class="icon-edit" aria-hidden="true"></i></a>' . "\r\n";
			$output .= '<a href="#" title="' . __( 'Delete', 'sp-theme' ) . '" class="module-element-delete"><i class="icon-remove" aria-hidden="true"></i></a>' . "\r\n";
			...
			$output .= '<div class="module-values">' . "\r\n";
			$output .= '<h3>' . $shortcode_name . '</h3>' . "\r\n";
			$output .= '<textarea name="pb[row][][column][][module][][custom_content][content]" class="pb-textarea module-value widefat">' . stripslashes( $content ) . '</textarea>' . "\r\n";
		...
		case 'placeholder' :
			$output = '<div class="module-element column-placeholder" data-id="' . esc_attr( $module ) . '">' . "\r\n";
			$output .= '<span class="module-element-name"></span>' . "\r\n";
			$output .= '<a href="#" title="' . __( 'Edit', 'sp-theme' ) . '" class="module-element-edit"><i class="icon-edit" aria-hidden="true"></i></a>' . "\r\n";
			$output .= '<a href="#" title="' . __( 'Delete', 'sp-theme' ) . '" class="module-element-delete"><i class="icon-remove" aria-hidden="true"></i></a>' . "\r\n";
		...
		default :
			$output = '<div class="module-element" data-id="' . esc_attr( $module ) . '">' . "\r\n";
			$output .= '<span class="module-element-name">' . $shortcodes[$module]['title'] . '</span>' . "\r\n";
			$output .= '<a href="#" title="' . __( 'Edit', 'sp-theme' ) . '" class="module-element-edit"><i class="icon-edit" aria-hidden="true"></i></a>' . "\r\n";
			$output .= '<a href="#" title="' . __( 'Delete', 'sp-theme' ) . '" class="module-element-delete"><i class="icon-remove" aria-hidden="true"></i></a>' . "\r\n";
...
function sp_display_slide_media_object( $attachment_id, $post_id, $size = '' ) { 
	...
	$output = '';
	$output .= '<div class="slide-container ui-state-default" data-attachment-id="' . absint( $attachment_id ) . '" data-post-id="' . $post_id . '" title="' . esc_attr( $post_obj->post_title ) . '">' . "\r\n";
	...
	$output .= '<a href="#" title="' . __( 'Remove Media', 'sp-theme' ) . '" class="remove-media"><i class="icon-remove" aria-hidden="true"></i></a>' . "\r\n";
	$output .= '<span class="edit">' . __( 'edit', 'sp-theme' ) . '</span>' . "\r\n";
	$output .= '<img src="' . THEME_URL . 'images/admin/ajax-loader.gif" alt="' . esc_attr( 'Loading', 'sp-theme' ) . '" width="16" height="16" class="alert" />' . "\r\n";
	$output .= '</div><!--close .slide-container-->' . "\r\n";
...
function sp_display_slide_media( $post_id ) { 
	...
	while( $attachments->have_posts() ) : $attachments->the_post();
		$output .= '<div class="slide-container ui-state-default" data-attachment-id="' . absint( $post->ID ) . '" data-post-id="' . absint( $post_id ) . '" title="' . esc_attr( get_the_title() ) . '">' . "\r\n";
...
function sp_display_slide_media_details( $attachment_id, $post_id ) {
	...
	if ( $post_obj->post_mime_type == 'content/text' ) {
		...
		$output .= '<form action="#" method="post" data-post-id="' . $post_id . '" data-attachment-id="' . $attachment_id . '">' . "\r\n";

		// textarea
		$output .= '<p class="howto">' . __( 'Enter the content text you would like displayed.', 'sp-theme' ) . '</p>' . "\r\n";
		$output .= '<textarea name="content_text" class="widefat" rows="20">' . stripslashes( $content ) . '</textarea><br /><br />' . "\r\n";
		$output .= '<input type="submit" value="' . esc_attr__( 'Save', 'sp-theme' ) . '" name="save" class="save-slide-settings button button-primary" />' . "\r\n";	
		$output .= '</form>' . "\r\n";	
	...
	if ( $post_obj->post_mime_type == 'audio/mpeg' ) {
		$output = '';
			...
			$output .= __( 'URL Path: ', 'sp-theme' ) . wp_get_attachment_url( $attachment_id ) . "\r\n";
			$output .= '<p>' . __( 'Here are the current browser support for HTML 5 audio formats', 'sp-theme' ) . '</p>' . "\r\n";
			$output .= '<table class="html5-browser-compat">' . "\r\n";
			$output .= '<tr>' . "\r\n";
			$output .= '<th>' . __( 'Browser', 'sp-theme' ) . '</th>' . "\r\n";
			$output .= '<th>MP3</th>' . "\r\n";
			$output .= '<th>OGG</th>' . "\r\n";
			$output .= '<th>WAV</th>' . "\r\n";
			$output .= '</tr>' . "\r\n";
			$output .= '<tr>' . "\r\n";
			$output .= '<td>Internet Explorer 9+</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'NO', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'NO', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '</tr>' . "\r\n";
			$output .= '<tr>' . "\r\n";
			$output .= '<td>Chrome 6+</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '</tr>' . "\r\n";	
			$output .= '<tr>' . "\r\n";
			$output .= '<td>Firefox 3.6+</td>' . "\r\n";
			$output .= '<td>' . __( 'NO', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '</tr>' . "\r\n";
			$output .= '<tr>' . "\r\n";
			$output .= '<td>Safari 5+</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'NO', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '</tr>' . "\r\n";
			$output .= '<tr>' . "\r\n";
			$output .= '<td>Opera 10.6+</td>' . "\r\n";
			$output .= '<td>' . __( 'NO', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '<td>' . __( 'YES', 'sp-theme' ) . '</td>' . "\r\n";
			$output .= '</tr>' . "\r\n";											
	...
	if ( $post_obj->post_mime_type == 'video/mp4' ) {
		...
		if ( isset( $external_link ) && ! empty( $external_link ) ) {
			$output = '';
			$output .= '<p>' . __( 'There are no options for this slide media.', 'sp-theme' ) . '</p>' . "\r\n";
			...
			$output .= __( 'URL Path: ', 'sp-theme' ) . wp_get_attachment_url( $attachment_id ) . "\r\n";
			$output .= '<p>' . __( 'Here are the current browser support for HTML 5 video formats', 'sp-theme' ) . '</p>' . "\r\n";
			$output .= '<table class="html5-browser-compat">' . "\r\n";
			$output .= '<tr>' . "\r\n";
			$output .= '<th>' . __( 'Browser', 'sp-theme' ) . '</th>' . "\r\n";
			$output .= '<th>MP4</th>' . "\r\n";
	...
	$output = '';
	$output .= '<form action="#" method="post" data-post-id="' . esc_attr( $post_id ) . '" data-attachment-id="' . esc_attr( $attachment_id ) . '">' . "\r\n";
	...
	foreach( $slide_array as $key => $value ) {
		...
		switch( $value['field_type'] ) {
			...
			case 'textarea':
				$output .= '<textarea name="' . esc_attr( $key ) . '" rows="8" class="widefat">' . stripslashes( esc_attr( $setting ) ) . '</textarea>' . "\r\n";
				break;
