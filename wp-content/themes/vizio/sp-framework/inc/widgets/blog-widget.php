...
class SP_Blog_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {	
		...
		$output .= '<div class="blog-widget ' . esc_attr( $show_tabs ) . '">' . "\r\n";
		...
		if ( null !== $recent_posts ) {
			...
			// build the tab menu
			$tabs .= '<li><a href="#recent-posts" title="' . esc_attr( $tab_link_title ) . '" class="tab-link sp-tooltip" data-placement="top" data-toggle="tooltip"><i class="icon-edit" aria-hidden="true"></i> ' . $tab_title . '</a></li>' . "\r\n";

			$content .= '<div id="recent-posts" class="content-wrap">' . "\r\n";
			...
			foreach( $recent_posts as $post ) {
				...
				$content .= '<li>' . "\r\n";
				$content .= '<a href="' . esc_url( get_permalink( $post['ID'] ) ) . '" title="' . esc_attr( $post['post_title'] ) . '">' . "\r\n";
				$content .= '<img src="' . esc_url( $post_image['url'] ) . '" width="' . esc_attr( $post_image['width'] ) . '" height="' . esc_attr( $post_image['height'] ) . '" alt="' . esc_attr( $post_image['alt'] ) . '" />' . "\r\n";
				$content .= '<h3 class="post-title">' . $post['post_title'] . '</h3>' . "\r\n";
				$content .= '</a>' . "\r\n";
				$content .= '<em>' . $date . '</em>' . "\r\n";
				$content .= '</li>' . "\r\n";
			}
			...
			$content .= '</div><!--close #recent-posts-->' . "\r\n";
		}
		...
		if ( null !== $recent_comments ) {
			...
			// build the tab menu
			$tabs .= '<li><a href="#recent-comments" title="' . esc_attr( $tab_link_title ) . '" class="tab-link sp-tooltip" data-placement="top" data-toggle="tooltip"><i class="icon-comments" aria-hidden="true"></i> ' . $tab_title . '</a></li>' . "\r\n";

			$content .= '<div id="recent-comments" class="content-wrap">' . "\r\n";
			...
			foreach( $recent_comments as $comment ) {
				...
				$content .= '<li class="comment-wrap clearfix">' . "\r\n";
				$content .= '<i class="icon-comment" aria-hidden="true"></i> <p class="comment"><a href="' . esc_url( get_permalink( $comment->comment_post_ID ) ) . '" title="' . __( 'Comment On:', 'sp-theme' ) . ' ' . esc_attr( $comment->post_title ) . '">' . $comment_content . '</a>' . "\r\n";
				$content .= '<em>' . __( 'by', 'sp-theme' ) . ' ' . $comment->comment_author . '</em><em>' . $date . '</em></p>' . "\r\n";
				$content .= '</li>' . "\r\n";
			}

			$content .= '</ul>' . "\r\n";

			$content .= '</div><!--close #recent-comments-->' . "\r\n";
		}
		...
		if ( null !== $popular_posts ) {
			...
			// build the tab menu
			$tabs .= '<li><a href="#popular-posts" title="' . esc_attr( $tab_link_title ) . '" class="tab-link sp-tooltip" data-placement="top" data-toggle="tooltip"><i class="icon-star" aria-hidden="true"></i> ' . $tab_title . '</a></li>' . "\r\n";

			$content .= '<div id="popular-posts" class="content-wrap">' . "\r\n";
		...
		// set active tab
		$output .= '<input type="hidden" class="active-tab" value="' . esc_attr( $instance['active_tab'] ) . '" name="active_tab" />' . "\r\n";

		$output .= '</div><!--close .blog-widget-tabs-->' . "\r\n";
