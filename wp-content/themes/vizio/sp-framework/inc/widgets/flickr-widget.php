...
function _sp_get_flickr_stream( $widget_id, $api_key, $user_id, $count = '10' ) {
	...
	if ( false === ( $rsp_obj = get_transient( $widget_id ) ) ) {
		...
		// build url
		$url = sp_ssl_protocol() . '://api.flickr.com/services/rest/?' . implode( '&', $encoded_params );
...
class SP_Flickr_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {	
		...
		if ( ! empty( $title ) )
			echo $before_title . apply_filters( 'sp_flickr_widget_title_icon', ' <i class="icon-flickr-2" aria-hidden="true"></i> ' ) . $title . $after_title;
		...
		if ( is_array( $photos ) && $photos['photos']['total'] > 0 ) {
