	...
	function end_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( $depth === 0 ) {
			if ( $this->mega_menu_status === 'on' ) {
				...
				$output .= "\r\n" . '</div><!--close .sp-mega-menu-column-container-->' . "\r\n";
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $this->mega_menu_status === 'on' && $depth === 1 ) {
			...
			if ( $this->columns > $args->max_columns || ( get_post_meta( $item->ID, '_menu-item-sp-mega-menu-new-row', true ) === 'on' && $this->columns != 1 ) ) {
				...
				$output .= "\r\n" . '<li class="sp-mega-menu-row-divider"></li>' . "\r\n";
			}
			...
			if ( $this->mega_menu_status === 'on' && $depth > 2 ) {
				$item_output .= '<i class="menu-indicator-sub ' . esc_attr( apply_filters( 'sp_mega_menu_indicators_sub', 'icon-angle-right' ) ) . '" aria-hidden="true"></i> ' . "\r\n";
			}
			...
			if ( $children = get_posts( array( 'post_type' => 'nav_menu_item', 'nopaging' => true, 'numberposts' => 1, 'meta_key' => '_menu_item_menu_item_parent', 'meta_value' => $item->ID ) ) && $depth === 0 )
				$item_output .= ' <i class="menu-indicator-parent ' . esc_attr( apply_filters( 'sp_menu_indicators_parent', 'icon-angle-down' ) ) . '" aria-hidden="true"></i><span class="menu-arrow"></span>' . "\r\n";

			if ( $this->mega_menu_status !== 'on' ) {
				// check if any children items and is depth 1+
				if ( $children = get_posts( array( 'post_type' => 'nav_menu_item', 'nopaging' => true, 'numberposts' => 1, 'meta_key' => '_menu_item_menu_item_parent', 'meta_value' => $item->ID ) ) && $depth > 0 )
					$item_output .= ' <i class="menu-indicator-sub ' . esc_attr( apply_filters( 'sp_menu_indicators_sub', 'icon-angle-right' ) ) . '" aria-hidden="true"></i>' . "\r\n";
			}
...
class SP_Footer_Menu extends Walker {
	...
	function walk( $elements, $max_depth) {
		...
		foreach ( $top_level_elements as $e ) {
			...
			// check if we need to show phone and is the last menu item
			if ( $e->ID === $last_child->db_id && sp_get_option( 'show_footer_phone', 'is', 'on' ) && ! empty( $phone ) ) {

				// prep the phone number
				$phone_raw = str_replace( array( ' ', '-', '(', ')', '.' ), '', $phone );
				$output .= '<li> ' . apply_filters( 'sp_footer_phone_label', __( '| Tel:', 'sp-theme' ) ) . '<a href="tel:' . esc_attr( $phone_raw ) . '" class="phone">' . $phone . '</a></li>' . "\r\n";
				// below may be used in the future
				//$output .= '<li><a href="wtai://wp/mc;' . esc_attr( $phone_raw ) . '" class="phone">' . $phone . '</a></li>' . "\r\n";
			}			
		}
