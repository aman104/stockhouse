...
function sp_pagination( $pages = '', $range = 2 ) {  
	...
	if ( 1 != $pages ) {
	   ...
	   if ( $paged > 2 && $paged > $range + 1 && $showitems < $pages ) 
			$output .= '<a href="' . get_pagenum_link( 1 ) . '" class="inactive"><i class="icon-angle-left"></i></a>' . "\r\n";
	   
	   if ( $paged > 1 && $showitems < $pages ) 
			$output .= '<a href="' . get_pagenum_link( $paged - 1 ) . '" class="inactive"><i class="icon-angle-left"></i></a>' . "\r\n";
	
	   for ( $i=1; $i <= $pages; $i++ ) {
		   if ( 1 != $pages && ( ! ( $i >= $paged + $range + 1 || $i <= $paged - $range - 1 ) || $pages <= $showitems ) )
			   $output .= ( $paged == $i ) ? '<a href="#" title="' . esc_attr__( 'Current', 'sp-theme' ) . '" class="current">' . $i . '</a>' : '<a href="' . get_pagenum_link( $i ) . '" title="' . esc_attr__( 'Page', 'sp-theme' ) . '" class="inactive" >' . $i . '</a>' . "\r\n";
	   }
	
	   if ( $paged < $pages && $showitems < $pages ) 
			$output .= '<a href="' . get_pagenum_link( $paged + 1 ) . '" class="inactive"><i class="icon-angle-right"></i></a>' . "\r\n";  
	   
	   if ( $paged < $pages - 1 &&  $paged + $range - 1 < $pages && $showitems < $pages ) 
			$output .= '<a href="' . get_pagenum_link( $pages ) . '" class="inactive"><i class="icon-angle-right"></i></a>' . "\r\n";
