...
function sp_display_carousel_slider( $post_id ) {
	...
	if ( $slider_show_title === 'on' ) {
		...
		$output .= '<h3 style="' . esc_attr( $slider_title_style ) . '" class="slider-title entry-title"><span>' . $slider_title_text . '</span></h3>' . "\r\n";					
	}
	...
	if ( $slider_type === 'single' )
		$output .= '<div class="inner-container single ' . $overlay_position_class . '" style="' . esc_attr( $slider_bg_style ) . '">' . "\r\n";
	else
		$output .= '<div class="inner-container carousel">' . "\r\n";
		...
		if ( $slider_type === 'single' ) {
			...
			switch( substr( $mime_type, 0, strpos( $mime_type, '/' ) ) ) {
				case 'image' :
					...
					if ( isset( $slide_content ) && ! empty( $slide_content ) ) {
						...
						$output .= '<div class="textblock-container row ' . $overlay_position . '">' . "\r\n";
						...
						if ( isset( $slide_slogan_title ) && $slide_slogan_title != '' ) {
							...
							$output .= '<h3 style="' . esc_attr( $title_styles ) . '" class="slogan-title">' . $slide_slogan_title . '</h3>' . "\r\n";
						}

						$output .= '<p style="' . esc_attr( $content_styles ) . '">' . $slide_content . '</p>' . "\r\n";
						...
						$output .= '</div><!--close .textblock-->' . "\r\n";

						$output .= '</div><!--close .textblock-container-->' . "\r\n";
					}
				...
				case 'video' :
					...
					if ( isset( $external_link ) && ! empty( $external_link ) ) {
						...
						$output .= '<video class="video" controls="controls" id="video-' . get_the_ID() . '">' . "\r\n";
						...
						if ( file_exists( substr( get_attached_file( get_the_id() ), 0, strrpos( get_attached_file( get_the_id() ), '.' ) ) . '.ogv' ) )
							$output .= '<source src="' . $video_link_ogg . '" type="video/ogv">' . "\r\n";
						...
						$output .= '<p>' . __( 'Sorry! Your browser does not support HTML 5 video.  Please use a more modern browser!', 'sp-theme' ) . '</p>' . "\r\n";
						$output .= '</video>' . "\r\n";
				...
				case 'audio' :
					...
					$output .= '<audio class="audio" controls="controls" id="audio-' . get_the_ID() . '">' . "\r\n";
					$output .= '<source src="' . $audio_link_mpeg . '"  type="audio/mpeg" />' . "\r\n";

					// checks if file exists then try to load
					if ( file_exists( substr( get_attached_file( get_the_id() ), 0, strrpos( get_attached_file( get_the_id() ), '.' ) ) . '.ogg' ) )
						$output .= '<source src="' . $audio_link_ogg . '"  type="audio/ogg" />' . "\r\n";

					// checks if file exists then try to load
					if ( file_exists( substr( get_attached_file( get_the_id() ), 0, strrpos( get_attached_file( get_the_id() ), '.' ) ) . '.wav' ) )
						$output .= '<source src="' . $audio_link_wav . '"  type="audio/wav" />' . "\r\n";

					$output .= '<p>' . __( 'Sorry! Your browser does not support HTML 5 audio.  Please use a more modern browser!', 'sp-theme' ) . '</p>' . "\r\n";
					$output .= '</audio>' . "\r\n";	
			...
			switch( substr( $mime_type, 0, strpos( $mime_type, '/' ) ) ) {
				case 'image' :
					...
					// add anchor tag if link is on and set
					if ( $slide_link_lightbox === 'on' )
						$output .= '<i class="icon-resize-full" aria-hidden="true"></i></a>' . "\r\n";				
					elseif ( $slide_link_out === 'on' && isset( $slide_link_url ) && ! empty( $slide_link_url ) && $slide_link_lightbox === 'off' )
						$output .= '<i class="icon-link" aria-hidden="true"></i></a>' . "\r\n";	
				...
				case 'video' :
					...
					if ( isset( $external_link ) && ! empty( $external_link ) ) {
						...
						$output .= '<video class="video" controls="controls" id="video-' . get_the_ID() . '" style="' . esc_attr( $video_styles ) . '">' . "\r\n";
				...
				case 'audio' :
					...
					$output .= '<audio class="audio" controls="controls" id="audio-' . get_the_ID() . '" style="' . esc_attr( $audio_styles ) . '">' . "\r\n";
					$output .= '<source src="' . $audio_link_mpeg . '"  type="audio/mpeg" />' . "\r\n";
				...
				case 'content' :
					...
					// show this if it is a content slide
					$output .= '<div class="content-text" style="' . esc_attr( $content_text_styles ) . '">' . "\r\n";
					$output .= $content_text . "\r\n";
					$output .= '</div><!--close .content-->' . "\r\n";
			...
			if ( $slide_link_out === 'on' && isset( $slide_link_url ) && ! empty( $slide_link_url ) ) {
				...
				$output .= '<h2 style="' . esc_attr( $link_styles ) . '">' . $slide_title . '</h2>' . "\r\n";
			}	
		}

		$output .= '</div><!--close .slide-->' . "\r\n";
	...
	if ( $slider_type === 'single' && $show_overlay ) {
		if ( isset( $overlay_color ) && ! empty( $overlay_color ) && isset( $overlay_opacity ) && ! empty( $overlay_opacity ) ) {
			...
			$output .= '<div class="overlay" style="' . esc_attr( $overlay_styles ) . '"></div>' . "\r\n";
		}
	}

	$output .= '</div><!--close .inner-container-->' . "\r\n";

	$output .= '<input type="hidden" name="slider_type" value="' . esc_attr( $slider_type ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_mode" value="' . esc_attr( $slider_mode ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_show_text_overlay" value="' . esc_attr( $show_overlay ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_overlay_position" value="' . esc_attr( $overlay_position ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_carousel_item_width" value="' . esc_attr( $carousel_item_width ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_easing" value="' . esc_attr( $slider_easing ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_randomize" value="' . esc_attr( $slider_randomize ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_autoscroll" value="' . esc_attr( $slider_autoscroll ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_interval" value="' . esc_attr( $slider_interval ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_circular" value="' . esc_attr( $slider_circular ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_nav" value="' . esc_attr( $slider_nav ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_dot_nav" value="' . esc_attr( $slider_dot_nav ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_pause_on_hover" value="' . esc_attr( $slider_pause_on_hover ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_items_per_click" value="' . esc_attr( $slider_items_per_click ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_transition_speed" value="' . esc_attr( $slider_transition_speed ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_reverse_direction" value="' . esc_attr( $slider_reverse_direction ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_touchswipe" value="' . esc_attr( $slider_touchswipe ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="slider_items_to_show" value="' . esc_attr( $slider_items_to_show ) . '" />' . "\r\n";
