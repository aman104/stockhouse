...
function sp_submit_contact_form_ajax() {
	...
	$from_email		= get_post_meta( $cf_id, '_sp_contact_form_from_email', true );
    ...
    $headers = sprintf( __( '%s', 'sp-theme' ) . ' ' . get_bloginfo( 'name' ) . ' | ' . $header_text . ' <' . $from_email . '>', 'From:' ) . PHP_EOL;  
...
function sp_submit_register_form_ajax() {
	...
	if ( !isset( $form_data['tos'] )) {
			$error_msg['tos'] = apply_filters( 'sp_sc_register_form_not_exist_tos', __( 'Zgoda na przetwarzanie jest wymagana do rejestracji!', 'sp-theme' ) );
			$proceed = false;
	}
	...
	// if wrong phone number
	if ( isset( $form_data['phone'] ) && $form_data['phone'] !== '' ) {
		if ( strlen( $form_data['phone'] ) < 9 ) {
			$error_msg['phone'] = apply_filters( 'sp_sc_register_form_phone_not_confirmed_msg', __( 'Podaj poprawny numer telefonu.', 'sp-theme' ) );
			$proceed = false;
		}
	}
	...
	if ( $proceed ) {
		...
		if ( is_int( $user_id ) ) {
			...
			// build email
			$to = $form_data['email'];
			$subject = get_bloginfo( 'name' ) . ' ' . __( 'Registration', 'sp-theme' );
			...
			$headers = sprintf( __( '%s', 'sp-theme' ) . ' ' . get_bloginfo( 'name' ) . ' <' . $email . '>', 'From:' ) . PHP_EOL;
