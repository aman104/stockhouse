...
function sp_social_media_profile_icons() {
	...
	$output .= '<ul class="social-media-profile-buttons">' . "\r\n";	
	
	if ( sp_get_option( 'facebook_enable', 'is', 'on' ) )
		$buttons['facebook'] = '<li class="facebook"><a href="' . esc_url( sp_get_option( 'facebook_account' ) ) . '" title="' . esc_attr__( 'Join Us on Facebook', 'sp-theme' ) . '" class="sp-tooltip" data-toggle="tooltip" data-placement="bottom"><i class="icon-' . apply_filters( 'sp_social_media_facebook_profile_icon_class', 'facebook' ) . '" aria-hidden="true"></i></a></li>' . "\r\n";

	if ( sp_get_option( 'twitter_enable', 'is', 'on' ) ) 
		$buttons['twitter'] = '<li class="twitter"><a href="' . esc_url( sp_get_option( 'twitter_account' ) ) . '" title="' . esc_attr__( 'Follow our Tweets', 'sp-theme' ) . '" class="sp-tooltip" data-toggle="tooltip" data-placement="bottom"><i class="icon-' . apply_filters( 'sp_social_media_twitter_profile_icon_class', 'twitter' ) . '" aria-hidden="true"></i></a></li>' . "\r\n";	

	if ( sp_get_option( 'pinterest_enable', 'is', 'on' ) )  
		$buttons['pinterest'] = '<li class="pinterest"><a href="' . esc_url( sp_get_option( 'pinterest_account' ) ) . '" title="' . esc_attr__( 'Follow our Pins', 'sp-theme' ) . '" class="sp-tooltip" data-toggle="tooltip" data-placement="bottom"><i class="icon-' . apply_filters( 'sp_social_media_pinterest_profile_icon_class', 'pinterest' ) . '" aria-hidden="true"></i></a></li>' . "\r\n";	

	if ( sp_get_option( 'flickr_enable', 'is', 'on' ) ) 
		$buttons['flickr'] = '<li class="flickr"><a href="' . esc_url( sp_get_option( 'flickr_account' ) ) . '" title="' . esc_attr__( 'Checkout our Flickr Photos', 'sp-theme' ) . '" class="sp-tooltip" data-toggle="tooltip" data-placement="bottom"><i class="icon-' . apply_filters( 'sp_social_media_flickr_profile_icon_class', 'flickr' ) . '" aria-hidden="true"></i></a></li>' . "\r\n";	

	if ( sp_get_option( 'gplus_enable', 'is', 'on' ) ) 
		$buttons['gplus'] = '<li class="gplus"><a href="' . esc_url( sp_get_option( 'gplus_account' ) ) . '" title="' . esc_attr__( 'Checkout our Google Plus Profile', 'sp-theme' ) . '" class="sp-tooltip" data-toggle="tooltip" data-placement="bottom"><i class="icon-' . apply_filters( 'sp_social_media_gplus_profile_icon_class', 'google-plus' ) . '" aria-hidden="true"></i></a></li>' . "\r\n";	

	if ( sp_get_option( 'youtube_enable', 'is', 'on' ) ) 
		$buttons['youtube'] = '<li class="youtube"><a href="' . esc_url( sp_get_option( 'youtube_account' ) ) . '" title="' . esc_attr__( 'Checkout our YouTube Videos', 'sp-theme' ) . '" class="sp-tooltip" data-toggle="tooltip" data-placement="bottom"><i class="icon-' . apply_filters( 'sp_social_media_youtube_profile_icon_class', 'youtube' ) . '" aria-hidden="true"></i></a></li>' . "\r\n";	
	
	if ( sp_get_option( 'rss_enable', 'is', 'on' ) ) 
		$buttons['rss'] = '<li class="rss"><a href="' . esc_url( get_bloginfo( 'rss2_url' ) ) . '" title="' . esc_attr__( 'Get Fed on our Feeds' , 'sp-theme' ) . '" class="sp-tooltip" data-toggle="tooltip" data-placement="bottom"><i class="icon-' . apply_filters( 'sp_social_media_rss_profile_icon_class', 'rss' ) . '" aria-hidden="true"></i></a></li>' . "\r\n";	
...
function sp_social_media_action_buttons( $size = 'small' ) {
	...
	$output .= '<ul class="social-action-buttons ' . $size . '">' . "\r\n";

	if ( $size === 'big' ) {
		if ( sp_get_option( 'facebook_enable', 'is', 'on' ) )
			$buttons['facebook'] = '<li>' . do_shortcode( '[sp-fblike layout="box_count"]' ) . '</li>' . "\r\n";

		if ( sp_get_option( 'twitter_enable', 'is', 'on' ) )
			$buttons['twitter'] = '<li>' . do_shortcode( '[sp-tweet count_position="vertical"]' ) . '</li>' . "\r\n";

		if ( sp_get_option( 'gplus_enable', 'is', 'on' ) )
			$buttons['gplus'] = '<li>' . do_shortcode( '[sp-gplusone size="tall"]' ) . '</li>' . "\r\n";

		if ( sp_get_option( 'pinterest_enable', 'is', 'on' ) )
			$buttons['pinterest'] = '<li>' . do_shortcode( '[sp-pinit count_layout="vertical"]' ) . '</li>' . "\r\n";
	} else {
		if ( sp_get_option( 'facebook_enable', 'is', 'on' ) )
			$buttons['facebook'] = '<li>' . do_shortcode( '[sp-fblike]' ) . '</li>' . "\r\n";

		if ( sp_get_option( 'twitter_enable', 'is', 'on' ) )
			$buttons['twitter'] = '<li>' . do_shortcode( '[sp-tweet]' ) . '</li>' . "\r\n";
		
		if ( sp_get_option( 'gplus_enable', 'is', 'on' ) )
			$buttons['gplus'] = '<li>' . do_shortcode( '[sp-gplusone]' ) . '</li>' . "\r\n";

		if ( sp_get_option( 'pinterest_enable', 'is', 'on' ) )
			$buttons['pinterest'] = '<li>' . do_shortcode( '[sp-pinit]' ) . '</li>' . "\r\n";
	}
...
function sp_display_logo() {
	...
	$output .= '<a href="' . esc_url( home_url() ) . '" title="' . esc_attr( 'Home', 'sp-theme' ) . '" id="logo">' . "\r\n";
	...
	if ( $logo_type === 'text' ) {
		...
		$output .= '<img src="' . esc_url( $logo_url ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" />' . "\r\n";
	}
...
function sp_display_tagline() {
	...
	$output .= '<em id="tagline">' . $tagline . '</em>' . "\r\n";
...
function _sp_display_favicon() { 
	...
	$output = '';
	$output .= '<link rel="shortcut icon" id="favicon" type="image/x-icon" href="' . esc_url( $favicon ) . '" />' . "\r\n";
...
function sp_copyright_html() { 
	...
	$output = '';
	$output .= '<small id="copyright">' . str_replace( array( '%%YEAR%%', '%YEAR%' ), date( 'Y' ), $info ) . '</small>' . "\r\n";
...
function sp_display_page_builder() {
	...
	if ( is_array( $pb_modules ) ) {
		...
		$output .= '<div class="page-builder main-container entry-content">' . "\r\n";
		...
		if ( ! post_password_required() ) {
			...
			foreach ( $pb_modules['row'] as $row ) {
				...
				// if showing last row display sidebar if needed
				if ( $row_count === $r ) {
					// make sure row is not in full width or else sidebar can't show
					if ( isset( $row['row_width'] ) && $row['row_width'] === 'boxed_width' ) {
						if ( $orientation === 'sidebar-left-layout' || $orientation === 'sidebar-right-layout' || $orientation === 'both-sidebars-layout' ) {
							$output .= '<div class="row ' . esc_attr( $orientation ) . '">' . "\r\n";
						}
						...
						if ( $orientation === 'sidebar-left-layout' || $orientation === 'sidebar-right-layout' || $orientation === 'both-sidebars-layout' ) {
							$output .= '<div class="' . esc_attr( $span_columns ) . '">' . "\r\n";
						}			
					}
				} else {
					if ( isset( $row['row_width'] ) && $row['row_width'] === 'box_width' )
				...
				if ( isset( $row['column'] ) ) {
					foreach( $row['column'] as $column ) { 
						$i = 1;
							...
							$output .= '<div class="' . esc_attr( $header_columns_class ) . '">' . "\r\n";
			...
			$output .= '<div class="container protected"><br /><br />' . "\r\n";
...
function sp_back_to_top_html() { 
	...
	if ( isset( $show ) && $show === 'on' )
		$output = '<a href="#top" class="site-btt" title="' . esc_attr__( 'Back to Top', 'sp-theme' ) . '"><i class="icon-chevron-up" aria-hidden="true"></i></a>' . "\r\n";
...
function sp_share_button_html() {
	...
	$output .= apply_filters( 'sp_share_button_link_html', '<a href="#" class="sp-tooltip share-button" title="' . esc_attr( 'Share This', 'sp-theme' ) . '" data-toggle="tooltip" data-placement="top"><i class="icon-group" aria-hidden="true"></i><span class="text">' . __( 'Share', 'sp-theme' ) . '</span></a>' ) . "\r\n";

	return $output;
}
endif;
/** Funkcja cena jednej sztuki 
*
*
*
*/
function jedna_sztuka() {
	$sztuka = get_post_meta( get_the_ID(), __( 'Cena jednej sztuki' ), true, 25 );
	if ( $sztuka )

   	 echo "<div class='box'>$sztuka</div>";
