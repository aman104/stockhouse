...
function sp_gplusonebutton_shortcode( $atts, $content = null ) {
	...
	$output = '';
	$output .= '<div class="sc-gplusone g-plusone" data-href="' . esc_url_raw( $url ) . '" data-size="' . esc_attr( $size ) . '" data-annotation="' . esc_attr( $annotation ) . '"></div><!--close .g-plusone-->' . "\r\n";
...
function sp_fblikebutton_shortcode( $atts, $content = null ) {
	...
	$output = '';
	$output .= '<div class="sc-fblike fb-like" data-href="' . esc_url_raw( $url ) . '" data-send="' . esc_attr( $send_button ) . '" data-layout="' . esc_attr( $layout ) . '" data-width="' . esc_attr( (int)$width ) . '" data-show-faces="' . esc_attr( $show_faces ) . '" data-action="' . esc_attr( $verb ) . '"></div><!--close .fb-like-->' . "\r\n";

	// check if action has already been added to prevent adding multiple times
	if ( ! has_action( 'wp_footer', 'sp_facebook_script' ) ) 
		add_action( 'wp_footer', 'sp_facebook_script' ); 

	return $output;
}

/**
 * Facebook Like Button js script
 *
 * @access public
 * @since 3.0
 * @return js
...
function sp_tweetbutton_shortcode( $atts, $content = null ) {
	...
	$output = '';	
	$output .= '<div class="sc-tweet">' . "\r\n";
	$output .= '<a href="' . esc_url( $twitter_url ) . '" class="' . esc_attr( $type_class ) . '" data-url="' . esc_url_raw( $url ) . '" data-count="' . esc_attr( $count_position ) . '" data-size="' . esc_attr( $size ) . '">' . $tweet . '</a>' . "\r\n";	
	$output .= '</div><!--close .tweet-sc-->' . "\r\n";

	// check if action has already been added to prevent adding multiple times
	if ( ! has_action( 'wp_footer', 'sp_twitter_script' ) ) 
		add_action( 'wp_footer', 'sp_twitter_script' );

	return $output;
}

/**
 * Twitter Tweet js script
 *
 * @access public
 * @since 3.0
 * @return js
 */
function sp_twitter_script() { ?>
...
function sp_twitterfollow_shortcode( $atts, $content = null ) {
	...
	$output .= '<a href="https://twitter.com/' . esc_attr( $user ) . '" class="twitter-follow-button" data-show-count="false" data-size="' . esc_attr( $size ) . '" data-show-screen-name="' . esc_attr( $show_username ) . '" data-dnt="true">' . __( 'Follow', 'sp-theme' ) . ' @' . $user . '</a>' . "\r\n";
...
function sp_pinit_shortcode( $atts, $content = null ) {
	...
	$output = '';
	$output .= '<div class="sc-pinit">' . "\r\n";
	$output .= '<a href="' . esc_url( $pinterest_url ) . '" class="pin-it-button" count-layout="' . esc_attr( $count_layout ) . '"><img border="0" src="//assets.pinterest.com/images/PinExt.png" title="' . esc_attr__( 'Pin It', 'sp-theme' ) . '" /></a>' . "\r\n";
	$output .= '</div><!--close .sc-pinit-->' . "\r\n";
	
	// check if action has already been added to prevent adding multiple times
	if ( ! has_action( 'wp_footer', 'sp_pinit_script' ) ) 
		add_action( 'wp_footer', 'sp_pinit_script' ); 

	return $output;
}

/**
 * Pinterest Button js script
 *
 * @access public
 * @since 3.0
 * @return js
 */
function sp_pinit_script() { ?>
...
function sp_grid_shortcode( $atts, $content = null ) {
	...
	if ( $first == 'true')
		$output .= '<div class="sc-grid row">' . "\r\n";

	$output .= '<div class="' . esc_attr( sp_column_css( '', '', '', (string)$span, '', '', (string)$offset ) ) . '">' . do_shortcode( $content ) . '</div>' . "\r\n";
...
function sp_hr_shortcode( $atts, $content = null ) {
	...
	$output = '';
	$output .= '<hr class="sc-hr ' . esc_attr( $custom_class ) . '" style="border-top-color:#' . esc_attr( $color ) . ';border-top-width:' . esc_attr( $thickness ) . 'px;width:' . esc_attr( $width ) . ';border-top-style:' . esc_attr( $style ) . ';background:transparent;" />' . "\r\n";
...
function sp_back_to_top( $atts, $content = null ) {
	...
	$output = '';
	$output .= '<div class="sc-btt clearfix ' . esc_attr( $animate ) . ' ' . esc_attr( $position ) . ' ' . esc_attr( $custom_class ) . '">' . "\r\n";
	
	if ( $style === '1' )
		$output .= '<a href="#top" title="' . esc_attr__( 'Top', 'sp-theme' ) . '" style="color:#' . esc_attr( $text_color ) . ';background-color:#' . esc_attr( $bg_color ) . ';" class="style-1"><i class="icon-chevron-up" aria-hidden="true"></i>' . __( 'Top', 'sp-theme' ) . '</a>' . "\r\n";
	else
		$output .= '<a href="#top" title="' . esc_attr__( 'Top', 'sp-theme' ) . '" style="color:#' . esc_attr( $text_color ) . ';background-color:#' . esc_attr( $bg_color ) . ';" class="style-2">' . __( 'Top', 'sp-theme' ) . ' <i class="icon-chevron-up" aria-hidden="true"></i></a>' . "\r\n";

	$output .= '</div><!--close .sc-btt-->' . "\r\n";
...
function sp_portfolio_shortcode( $atts, $content = null ) {
	...
	if ( $sort_order !== 'name' ) {
		$metakey = '_sp_portfolio_sort_order';
	...
	$output = '';
	$output .= '<div class="sc-portfolio clearfix container-fluid ' . esc_attr( $custom_class ) . '">' . "\r\n";
	...
	if ( $show_cat_description === 'true' ) {
		...
		if ( is_object( $cat ) && strlen( $cat->description ) > 0 ) {
			...
			$output .= '</div><!--close .cat-description-->' . "\r\n";
		}
	}
	...
	if ( $show_filters === 'true' ) {
		...
		if ( is_array( $filtered_tags ) && strlen( (string)$filtered_tags ) ) {
			$output .= '<ul class="portfolio-sort clearfix">' . "\r\n";

			// display the all filter
			$output .= '<li><a href="#" title="' . esc_attr__( 'All', 'sp-theme' ) . '" data-filter="*" class="active">' . __( 'All', 'sp-theme' ) . '</a></li>' . "\r\n";

			foreach ( $filtered_tags as $tag ) {
				$output .= '<li><span class="divider"> / </span><a href="#" title="' . esc_attr( $tag ) . '" data-filter=".' . esc_attr( str_replace( ' ', '-', $tag ) ) . '-sort">' . $tag . '</a></li>' . "\r\n";	
			}
	...
	if ( $isotope ) {
		$output .= '<div class="isotope-wrap center">' . "\r\n";
	}
	...
	while ( $entries->have_posts() ) : $entries->the_post();
		...
		if ( $mosaic === 'true' ) {
			$output .= '<article id="post-' . esc_attr( $post->ID ) . '" class="portfolio-item ' . esc_attr( $tags ) . ' mosaic">' . "\r\n";

			$output .= '<a href="' . esc_url( $image_full['url'] ) . '" title="' . the_title_attribute( 'echo=0' ) . '" class="portfolio-image-link mfp-image" data-rel="sp-lightbox[' . esc_attr( empty( $gal_id ) ? $post->ID : $gal_id ) . ']">' . "\r\n";
			
			$output .= '<img src="' . esc_url( $image_thumbnail['url'] ) . '" alt="' . esc_attr( $image_thumbnail['alt'] ) . '" />' . "\r\n";
			
			$output .= '<i class="hover-icon icon-resize-full" aria-hidden="true"></i></a>' . "\r\n";
		...
		if ( ! $isotope ) {
			if ( (int)( $entries->current_post + 1 ) === 1 )
				$output .= '<div class="items row center">' . "\r\n";
		}

		$output .= '<article id="post-' . esc_attr( $post->ID ) . '" class="portfolio-item ' . esc_attr( $span ) . ' ' . esc_attr( $tags ) . '">' . "\r\n";
		
		// if show gallery only
		if ( $gallery_only === 'true') {    

			$output .= '<figure class="image-wrap gallery-only" style="max-width:' . esc_attr( (int)$image_width ) . 'px;">' . "\r\n";
			...
			$output .= '</figure><!--close .image-wrap-->' . "\r\n";
		} else {
			$output .= '<figure class="image-wrap" style="max-width:' . esc_attr( (int)$image_width ) . 'px;">' . "\r\n";
			
			$output .= '<a href="' . get_permalink() .'" title="' . esc_attr__( 'Detail', 'sp-theme' ) . '">' . "\r\n";
			
			$output .= '<img src="' . esc_url( $image_thumbnail['url'] ) . '" alt="' . esc_attr( $image_thumbnail['alt'] ) . '" />' . "\r\n";
			
			$output .= '<i class="hover-icon icon-link" aria-hidden="true"></i></a>' . "\r\n";
			
			$output .= '<figcaption>' . "\r\n";
				
			// check if show title is on
			if ( isset( $show_title ) && $show_title === 'true' ) { 
				$output .= '<h2 class="entry-title"><a href="' . get_permalink() . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'sp-theme' ), the_title_attribute( 'echo=0' ) ) . '" data-rel="bookmark">' . get_the_title() . '</a></h2>' . "\r\n";
			}	
	...
	if ( $isotope )
		$output .= '</div><!--close .isotope-wrap-->' . "\r\n";

	//$output .= sp_pagination( $entries->max_num_pages ) . "\r\n";
	$output .= '<input type="hidden" name="link_gallery" value="' . esc_attr( $link_gallery ) . '" class="link-gallery" />' . "\r\n";
	$output .= '</div><!--close .sc-portfolio-->' . "\r\n";
...
function sp_testimonial_shortcode( $atts, $content = null ) {
	...
	$output .= '<div class="sc-testimonial clearfix ' . esc_attr( $custom_class ) . '">' . "\r\n";
	...
	if ( isset( $submit_testimonial ) && $submit_testimonial === 'false' ) {
		...
		while ( $entries->have_posts() ) : $entries->the_post();
			...
			$output .= '<article id="post-' . esc_attr( $post->ID ) . '" class="testimonial-item row ' . esc_attr( $avatar_class ) . ' ' . esc_attr( $quote_marks ) . '">' . "\r\n";
			
			if ( $show_avatar === 'true' )
				$output .= '<div class="avatar-wrap">' . $avatar . '</div><!--close .avatar-wrap-->' . "\r\n";

			// only show if show quote marks is on and avatar is off
			if ( $show_avatar === 'false' && $show_quote_marks === 'true' )
				$output .= '<i class="icon-quotes-left" aria-hidden="true"></i>' . "\r\n";

			$output .= '<blockquote>' . "\r\n";
			$output .= '<p>' . strip_tags( get_the_content() ) . '</p>' . "\r\n";
		...
		// check if autorotate is on
		if ( $autorotate === 'true' )
			$output .= '<input type="hidden" class="interval" value="' . esc_attr( $autorotate_interval ) . '" />' . "\r\n";
		...
		if ( $show_title === 'true' )
			$output .= '<h3 class="entry-title">' . $title . '</h3>' . "\r\n";
		$output .= '<p><span class="required">' . apply_filters( 'sp_testimonial_form_sc_required_symbol', '*' ) . '</span> - ' . __( 'Required', 'sp-theme' ) . '</p>' . "\r\n";
		$output .= '<form action="#" method="post" role="form">' . "\r\n";
		$output .= '<h3 class="title"><i class="icon-comments-alt" aria-hidden="true"></i> ' . apply_filters( 'sp_sc_testimonial_submit_title', __( 'We would love your testimonial!', 'sp-theme' ) ) . '</h3>' . "\r\n";
		$output .= '<div class="form-group">' . "\r\n";
		$output .= '<label for="name" class="control-label field-label">' . __( 'Name', 'sp-theme' ) . ' <span class="required">' . apply_filters( 'sp_testimonial_form_sc_required_symbol', '*' ) . '</span></label>' . "\r\n";
		$output .= '<input type="text" id="name" name="testimonial_submitter_name" value="" data-required="required" class="form-control" />' . "\r\n";
		$output .= '</div><!--close .form-group-->' . "\r\n";

		$output .= '<div class="form-group">' . "\r\n";
		$output .= '<label for="email" class="control-label field-label">' . __( 'Email', 'sp-theme' ) . ' <span class="required">' . apply_filters( 'sp_testimonial_form_sc_required_symbol', '*' ) . '</span></label>' . "\r\n";
		$output .= '<input type="email" id="email" name="testimonial_submitter_email" value="" data-required="required" class="form-control" />' . "\r\n";
		$output .= '</div><!--close .form-group-->' . "\r\n";

		$output .= '<div class="form-group">' . "\r\n";
		$output .= '<label for="testimonial" class="control-label field-label">' . __( 'Testimonial', 'sp-theme' ) . ' <span class="required">' . apply_filters( 'sp_testimonial_form_sc_required_symbol', '*' ) . '</span></label>' .  "\r\n";
		$output .= '<textarea id="testimonial" name="testimonial" rows="10" data-required="required" class="form-control"></textarea>' . "\r\n";
		$output .= '</div><!--close .form-group-->' . "\r\n";

		$output .= '<div class="form-actions clearfix">' . "\r\n";
		$output .= '<button type="submit" name="submit" class="button" value="submit_testimonial"><i class="loader" aria-hidden="true"></i> ' . __( 'Send', 'sp-theme' ) . '</button>' . "\r\n";

		$output .= '</div><!--close .form-actions-->' . "\r\n";

		$output .= wp_nonce_field( 'sp_testimonial_submit', '_sp_testimonial_submit_nonce', true, false ) . "\r\n";
		$output .= '</form>' . "\r\n";
	}
	
	$output .= '</div><!--close .sc-testimonial-->' . "\r\n";
...
function sp_button_shortcode( $atts, $content = null ) {
	...
	if ( isset( $icon ) && ! empty( $icon ) && $icon != '' ) {
		$output .= '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $title ) . '" target="' . esc_attr( $target ) . '" class="sc-button ' . esc_attr( $style ) . ' ' . esc_attr( $position ) . ' ' . esc_attr( $size ) . '" style="color:#' . esc_attr( $text_color ) . ';background:url(' . get_template_directory_uri() . '/images/sc-button-overlay.png) repeat-x scroll center center #' . esc_attr( $button_color ) . '"><i class="' . esc_attr( $icon ) . '" aria-hidden="true"></i> ' . $title . '</a>' . "\r\n";
	} else {
		$output .= '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $title ) . '" target="' . esc_attr( $target ) . '" class="sc-button ' . esc_attr( $style ) . ' ' . esc_attr( $position ) . ' ' . esc_attr( $size ) . '" style="color:#' . esc_attr( $text_color ) . ';background:url(' . get_template_directory_uri() . '/images/sc-button-overlay.png) repeat-x scroll center center #' . esc_attr( $button_color ) . '">' . $title . '</a>' . "\r\n";
	}
...
function sp_lightbox_shortcode( $atts, $content = null ) {
	...
	$output .= '<div class="sc-lightbox ' . esc_attr( $custom_class ) . '">' . "\r\n";
	...
	if ( ! empty( $url ) ) {
		...
		if ( $source_url['host'] == 'www.youtube.com' || $source_url['host'] == 'youtube.com' ) {
			...
			if ( $poster_size == "large" || $poster_size == "small" ) {
				...
				$output .= '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $title ) . '" data-group="' . esc_attr( $group ) . '" class="lightbox ' . esc_attr( $mfp_type ) . '"><img src="' . esc_url( sp_ssl_protocol() . '://img.youtube.com/vi/' . $id . '/' . $size ) . '" alt="' . esc_attr( $title ) . '" /><i class="icon-resize-full" aria-hidden="true"></i></a>' . "\r\n";	
			} else {
				$output .= '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $title ) . '" data-group="' . esc_attr( $group ) . '" class="lightbox ' . esc_attr( $mfp_type ) . '">' . "\r\n";

				// if thumbnail is set
				if ( isset( $thumbnail ) && ! empty( $thumbnail ) )
					$output .= '<img src="' . esc_url( $thumbnail ) . '" alt="' . esc_attr( $title ) . '" class="thumbnail" />' . "\r\n";
				else
					$output .= '<img src="' . esc_url( sp_no_image( 'small' ) ) . '" alt="' . esc_attr_e( 'No Image', 'sp-theme' ) . '" class="thumbnail" />' . "\r\n";

				$output .= do_shortcode( $content ) . '<i class="icon-resize-full" aria-hidden="true"></i></a>' . "\r\n";		
			}
		...
		// check if source is from vimeo
			...
			if ( $poster_size == "large" || $poster_size == "small" ) {
				...
				$output .= '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $title ) . '" data-group="' . esc_attr( $group ) . '" class="lightbox ' . esc_attr( $mfp_type ) . '"><img src="' . $image . '" alt="' . esc_attr( $title ) . '" /><i class="icon-resize-full" aria-hidden="true"></i></a>' . "\r\n";		
			} else {
				$output = '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $title ) . '" data-group="' . esc_attr( $group ) . '" class="lightbox ' . esc_attr( $mfp_type ) . '">' . "\r\n";
		...
		$id = rand( 0, 1000 );
		$output .= '<a href="#content-' . esc_attr( $id ) . '" data-group="' . esc_attr( $group ) . '" class="lightbox ' . esc_attr( $mfp_type ) . '">' . $title . '</a>' . "\r\n";	
		$output .= '<div class="content mfp-hide" id="content-' . esc_attr( $id ) . '" style="display:none;">' . do_shortcode( $content ) . '</div><!--close .content-->' . "\r\n";
	}

	$output .= '</div><!--close sc-lightbox-->' . "\r\n";	
...
function sp_google_map_shortcode( $atts, $content = null ) {
	...
	$output .= '<div class="sc-google-map">' . "\r\n";

	if ( $static === 'true' ) {

		$output .= '<div class="map"><img src="' . sp_ssl_protocol() . '://maps.apple.com/maps/api/staticmap?center=' . urlencode( $address ) . '&amp;zoom=' . esc_attr( $zoom ) . '&amp;size=' . esc_attr( $width ) . 'x' . esc_attr( $height ) . '&amp;sensor=false&amp;maptype=roadmap&amp;markers=color:red|' . urlencode( $address ) . '" alt="Google Map" /></div>' . "\r\n";	
	...
	if ( $directions_link === 'true' ) {
		$output .= '<a href="' . sp_ssl_protocol() . '://maps.apple.com/?q=' . urlencode( $address ) . '" title="' . esc_attr__( 'Map &amp; Directions', 'sp-theme' ). '" target="_blank" class="directions-link">' . __( 'Map &amp; Directions', 'sp-theme' ) . '</a>' . "\r\n";
	}

	$output .= '</div><!--close .sc-google-map-->' . "\r\n";
...
function sp_tabs_shortcode( $atts, $content = null ) {
	...
	$output .= '<div class="sc-tabs row ' . esc_attr( $custom_class ) . ' ' . esc_attr( $style ) . '" style="' . esc_attr( $width ) . '">' . "\r\n";
	
	// check if style is nav left
	if ( $style === 'nav-left' )
		$output .= '<div class="' . esc_attr( sp_column_css( '', '', '', '3' ) ) . '">' . "\r\n";
	...
	foreach ( $tab_titles as $title ) {
		...
		$output .= '<li>' . "\r\n";
		$output .= '<a href="#' . esc_attr( $title ) . '"><i class="' . esc_attr( $icon ) . '"></i> ' . esc_attr( str_replace( "-", " ", $title ) ) . '<i class="icon-angle-right"></i></a>' . "\r\n";
		$output .= '</li>' . "\r\n";
	...
	if ( $style === 'nav-left' ) {
		...
		$output .= '<div class="' . esc_attr( sp_column_css( '', '', '', '9' ) ) . '">' . "\r\n";
	}

	$output .= do_shortcode( $content ) . "\r\n";

	$output .= '<input type="hidden" class="collapsible" value="' . esc_js( $collapsible ) . '" />' . "\r\n";
	$output .= '<input type="hidden" class="active" value="' . esc_js( $active ) . '" />' . "\r\n";

	// check if style is nav left
	if ( $style === 'nav-left' )
		$output .= '</div><!--close .column-->' . "\r\n";

	$output .= '</div><!--close .sc-tabs-->' . "\r\n";
...
function sp_tabs_content_shortcode( $atts, $content = null ) {
	...
	$output .= '<div id="' . esc_attr( str_replace( ' ', '-', $tab ) ) . '" class="sc-tabs-container ui-tabs-hide" style="' . esc_attr( $style ) . '">' . "\r\n";
	$output .= do_shortcode( $content ) . "\r\n";
	$output .= '</div><!--close .tabs-container-->' . "\r\n";
...
function sp_carousel_shortcode( $atts, $contents = null ) {
	...
	$output .= '<div class="sc-carousel clearfix ' . esc_attr( $custom_class ) . '">' . "\r\n";

	$output .= sp_display_carousel_slider( $id ) . "\r\n";

	$output .= '</div><!--close .sc-carousel-->' . "\r\n";
...
function sp_accordion_shortcode( $atts, $contents = null ) {
	...
	$output = '';
	$output .= '<div class="sc-accordion ' . esc_attr( $custom_class ) . ' style-' . esc_attr( $style ) . '">' . "\r\n";
	$output .= do_shortcode( $contents ) . "\r\n";
	$output .= '<input type="hidden" name="collapsible" value="' . esc_js( $collapsible ) . '" />' . "\r\n";
	$output .= '<input type="hidden" name="active_panel" value="' . esc_js( $active_panel ) . '" />' . "\r\n";
	$output .= '</div>' . "\r\n";
...
function sp_accordion_content_shortcode( $atts, $contents = null ) {
	...
	$output = '';
	$output .= '<h3 class="clearfix"><span class="' . esc_attr( $icon ) . '"></span> ' . $title . '<i class="handle icon-angle-up" aria-hidden="true"></i></h3>' . "\r\n";
...
function sp_callout_shortcode( $atts, $contents = null ) {
	...
	$output .= '<div class="sc-callout ' . esc_attr( $type ) . ' ' . esc_attr( $custom_class ) . '">' . "\r\n";
	$output .= '<h3><i class="' . esc_attr( $icon ) . '" aria-hidden="true"></i> ' . $title . '</h3>' . "\r\n";
...
function sp_contact_shortcode( $atts, $contents = null ) {
	...
	$output .= '<div class="sc-contact-form ' . esc_attr( $custom_class ) . '">' . "\r\n";
	
	// form header
	if ( isset( $header_text ) && ! empty( $header_text ) )
		$output .= '<h2 class="form-header">' . $header_text . '</h2>' . "\r\n";

	// required field text
	if ( isset( $required_field_text ) && ! empty( $required_field_text ) )
		$output .= '<small class="required-field-text"><span class="required">' . apply_filters( 'sp_contact_form_sc_required_symbol', '*' ) . '</span> - ' . $required_field_text . '</small>' . "\r\n";
	...
	if ( $fields > 0 ) {
		...
		for ( $i = 1; $i <= $fields; $i++ ) {
			switch( $field_type['field_' . $i] ) {
				case 'text' :
					...
					$output .= '<div class="form-group">' . "\r\n";
					$output .= '<label for="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="field-label control-label">' . $field_label['field_' . $i] . ' ' . $show_required . '</label>' . "\r\n";
					$output .= '<input type="text" name="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" id="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" value="" class="form-control" ' . $required_attr . ' />' . "\r\n";
					$output .= '</div><!--close .form-group-->' . "\r\n";
					break;
				...
				case 'email' :
					...
					$output .= '<div class="form-group">' . "\r\n";
					$output .= '<label for="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="field-label control-label">' . $field_label['field_' . $i] . ' ' . $show_required . '</label>' . "\r\n";
					$output .= '<input type="email" name="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" id="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" value="" class="form-control" ' . $required_attr . ' />' . "\r\n";
					$output .= '</div><!--close .form-group-->' . "\r\n";
					break;
				...
				case 'textarea' :
					...
					$output .= '<div class="form-group">' . "\r\n";
					$output .= '<label for="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="field-label control-label">' . $field_label['field_' . $i] . ' ' . $show_required . '</label>' . "\r\n";
					$output .= '<textarea name="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" id="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" rows="10" class="form-control" ' . $required_attr . '></textarea>' . "\r\n";
					$output .= '</div><!--close .form-group-->' . "\r\n";
					break;
				...
				case 'radio' :
					...
					$output .= '<div class="form-group">' . "\r\n";
					$output .= '<label class="field-label control-label">' . $field_label['field_' . $i] . '</label>' . "\r\n";
					foreach( $field_options['field_' . $i] as $options ) {
						$output .= '<label for="' . esc_attr( strtolower( str_replace( ' ', '-', $options ) ) ) . '" class="options-label"><input type="radio" name="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" id="' . esc_attr( strtolower( str_replace( ' ', '-', $options ) ) ) . '" value="' . esc_attr( strtolower( str_replace( ' ', '_', $options ) ) ) . '" /> ' . $options . ' </label>' . "\r\n";
					}
					$output .= '</div><!--close .form-group-->' . "\r\n";
					break;
				...
				case 'checkbox' :
					...
					$output .= '<label for="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="field-label control-label"><input type="checkbox" name="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" id="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" value="true" /> ' . $field_label['field_' . $i] . ' </label>' . "\r\n";
					$output .= '</div><!--close .checkbox-->' . "\r\n";
					$output .= '</div><!--close .form-group-->' . "\r\n";
					break;
				...
				case 'select' :
					...
					$output .= '<div class="form-group">' . "\r\n";
					$output .= '<label for="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="field-label control-label">' . $field_label['field_' . $i] . ' ' . $show_required . '</label>' . "\r\n";
					$output .= '<select name="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" id="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="select2-select" data-placeholder="' . apply_filters( 'sp_contact_form_please_select_text', __( '--Please Select--', 'sp-theme' ) ) . '" data-no_results_text="' . esc_attr__( 'No Results', 'sp-theme' ) . '" ' . $required_attr . '>' . "\r\n";
					$output .= '<option value="0">' . apply_filters( 'sp_contact_form_please_select_text', __( '--Please Select--', 'sp-theme' ) ) . '</option>' . "\r\n";
					foreach( $field_options['field_' . $i] as $options ) {
						$output .= '<option value="' . esc_attr( strtolower( str_replace( ' ', '_', $options ) ) ) . '">' . $options . '</option>' . "\r\n";
					}						
				...
				case 'multiselect' :
					...
					$output .= '<div class="form-group">' . "\r\n";
					$output .= '<label for="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="field-label control-label">' . $field_label['field_' . $i] . ' ' . $show_required . '</label>' . "\r\n";
					$output .= '<select name="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" id="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="select2-select" multiple="multiple" data-placeholder="' . apply_filters( 'sp_contact_form_please_select_text', esc_attr__( '--Please Select--', 'sp-theme' ) ). '" ' . $required_attr . '>' . "\r\n";
				...
				case 'datepicker' :
					...
					$output .= '<div class="form-group">' . "\r\n";
					$output .= '<label for="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="field-label control-label">' . $field_label['field_' . $i] . ' ' . $show_required . '</label>' . "\r\n";
					$output .= '<input type="text" name="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" id="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" value="" class="form-control datepicker" ' . $required_attr . ' />' . "\r\n";
					$output .= '</div><!--close .form-group-->' . "\r\n";
					break;
				...
				case 'datetimepicker' :
					...
					$output .= '<div class="form-group">' . "\r\n";
					$output .= '<label for="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="field-label control-label">' . $field_label['field_' . $i] . ' ' . $show_required . '</label>' . "\r\n";
					$output .= '<input type="text" name="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" id="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" value="" class="form-control datetimepicker" ' . $required_attr . ' />' . "\r\n";
					$output .= '</div><!--close .form-group-->' . "\r\n";
					break;
				...
				case 'captcha' :
					...
					$output .= '<div class="form-group">' . "\r\n";
					$output .= '<img id="captcha" src="' . THEME_URL . 'securimage/securimage_show.php" alt="' . esc_attr__( 'CAPTCHA Image', 'sp-theme' ). '" />' . "\r\n";
					$output .= '<label for="' . esc_attr( $unique_tag_name['field_' . $i] ) . '" class="field-label control-label">' . $field_label['field_' . $i] . ' ' . $show_required . '</label>' . "\r\n";
				...
				case 'heading' :
					$output .= '<h3 class="section-heading">' . $field_label['field_' . $i ] . '</h3>' . "\r\n";
					break;
		...
		$output .= '<p class="alert alert-success"><button class="close" data-dismiss="alert" type="button">&times;</button>' . $success_message . '</p>' . "\r\n";
		$output .= '<p class="alert alert-warning"><button class="close" data-dismiss="alert" type="button">&times;</button>' . $failure_message . '</p>' . "\r\n";
		$output .= '<div class="form-actions clearfix">' . "\r\n";
		
		// check if we need to show reset
		if ( isset( $show_reset ) && $show_reset === 'on' )
			$output .= '<button type="reset" class="btn">' . $reset_button_text . '</button>' . "\r\n";

		$output .= '<button type="submit" class="btn btn-primary" name="submit" value="submit-contact-form"><i class="loader" aria-hidden="true"></i> ' . $submit_button_text . '</button>' . "\r\n";
		$output .= '</div><!--close .form-actions-->' . "\r\n";
	} // end have fields
	$output .= '<input type="hidden" name="cf_id" value="' . esc_attr( $id ) . '" />' . "\r\n";
	$output .= wp_nonce_field( 'sp_submit_contact_form', '_sp_submit_contact_form_nonce', true, false );
	$output .= '</form>' . "\r\n";
	$output .= '</div><!--close .sc-contact-form-->' . "\r\n";
...
function sp_faq_shortcode( $atts, $content = null ) {
	...
	$output .= '<div class="sc-faq ' . esc_attr( $custom_class ) . '">' . "\r\n";

	if ( isset( $title ) && ! empty( $title ) )
		$output .= '<h2 class="section-title"><i class="' . esc_attr( $icon ) . '" aria-hidden="true"></i> ' . $title . '</h2>' . "\r\n";

	while ( $faq->have_posts() ) : $faq->the_post();

		$output .= '<h3 class="entry-title"><i class="icon-plus" aria-hidden="true"></i> <a href="#" title="' . esc_attr( get_the_title() ) . '">' . get_the_title() . '</a></h3>' . "\r\n";
		$output .= '<div class="answer"><p>' . get_the_content() . '</p></div>' . "\r\n";

	endwhile;

	wp_reset_postdata();
	
	$output .= '<input type="hidden" name="collapsible" value="' . esc_attr( $collapsible ) . '" />' . "\r\n";
	$output .= '</div><!--close .sc-faq-->' . "\r\n";	
...
function sp_register_shortcode( $atts, $content = null ) {
	// extracts the attributes into variables
	extract( shortcode_atts( array(
		'firstname'			=> 'true', // true will allow visitors to input firstname
		'lastname'			=> 'true', // true will allow visitors to input lastname
		'captcha'			=> 'true',
		'phone'				=> 'true',
		'regulamin'			=> 'true',
		'tos'				=> 'true',
	...
	$output .= '<div class="sc-register-form ' . esc_attr( $custom_class ) . '">' . "\r\n";
	$output .= '<div class="heading clearfix">' . "\r\n";
	$output .= '<h3 class="title"><span>' . $title . '</span></h3>' . "\r\n";
	$output .= '<div class="meta clearfix">' . "\r\n";
	$output .= '<i class="icon-user" aria-hidden="true"></i><p class="tagline-wrapper clearfix"><span class="tagline">' . apply_filters( 'sp_sc_register_form_tagline', __( 'New Customer?', 'sp-theme' ) ) . '</span>' . "\r\n";
	$output .= '<span class="sub-tagline">' . apply_filters( 'sp_sc_register_form_sub_tagline', __( 'Please create your account.', 'sp-theme' ) ) . '</span></p>' . "\r\n";
	...
	if ( isset( $firstname ) && $firstname === 'true' ) {
		$output .= '<div class="form-group">' . "\r\n";
		$output .= '<label class="field-label control-label">' . __( 'First Name', 'sp-theme' ) . ' <span class="required">*</span></label>' . "\r\n";
		$output .= '<div class="controls">' . "\r\n";
		$output .= '<input type="text" name="firstname" class="form-control" data-required="required" tabindex="1" />' . "\r\n";
	...
	if ( isset( $lastname ) && $lastname === 'true' ) {
		$output .= '<div class="form-group">' . "\r\n";
		$output .= '<label class="field-label control-label">' . __( 'Last Name', 'sp-theme' ) . ' <span class="required">*</span></label>' . "\r\n";
		$output .= '<div class="controls">' . "\r\n";
		$output .= '<input type="text" name="lastname" class="form-control" data-required="required" tabindex="2" />' . "\r\n";
		$output .= '</div><!--close .controls-->' . "\r\n";
		$output .= '<p class="alert"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";
		$output .= '</div><!--close .form-group-->' . "\r\n";
	}
	...
	$output .= '<div class="controls">' . "\r\n";
	$output .= '<input type="text" name="email" class="form-control" data-required="required" tabindex="4" />' . "\r\n";
	$output .= '</div><!--close .controls-->' . "\r\n";
	$output .= '<p class="alert"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";
	$output .= '</div><!--close .form-group-->' . "\r\n";

	$output .= '<div class="form-group">' . "\r\n";
	$output .= '<label class="field-label control-label">' . __( 'Confirm Email', 'sp-theme' ) . ' <span class="required">*</span></label>' . "\r\n";
	$output .= '<div class="controls">' . "\r\n";
	$output .= '<input type="text" name="confirm_email" class="form-control" data-required="required" tabindex="5" />' . "\r\n";
	$output .= '</div><!--close .controls-->' . "\r\n";
	$output .= '<p class="alert"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";
	$output .= '</div><!--close .form-group-->' . "\r\n";
	...
	$output .= '<div class="form-group">' . PHP_EOL;
	$output .= '<label class="field-label control-label">' . __( 'Zaakceptuj', 'sp-theme' ) . ' <a href="http://stockhouse.com.pl/regulamin/">Regulamin</a> <span class="required">*</span></label>' . PHP_EOL;
	$output .= '<div class="controls">' . PHP_EOL;
	$output .= '<input type="checkbox" name="regulamin" class="form-control" data-required="required" tabindex="5" value="1"/>' . PHP_EOL;
	$output .= '</div><!--close .controls-->' . PHP_EOL;
	$output .= '<p class="alert"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . PHP_EOL;
	$output .= '</div><!--close .form-group-->' . PHP_EOL;

	$output .= '<div class="form-group">' . PHP_EOL;
	$output .= '<label class="field-label control-label">' . __( 'Wyrażam zgodę', 'sp-theme' ) . ' <span class="required">*</span></label>' . PHP_EOL;
	$output .= '<div class="controls">' . PHP_EOL;
	$output .= '<input type="checkbox" name="tos" class="form-control" data-required="required" tabindex="5" value="1"/>' . PHP_EOL;
	$output .= '</div><br />Wyrażam zgodę na przetwarzanie moich danych osobowych zgodnie z ustawą z dnia 29 sierpnia 1997 r. o ochronie danych osobowych (tekst jedn.: Dz. Ustaw z 2002 nr 101, poz. 926 z późn. zm.) dla potrzeb marketingowych firmy Stockhouse.com.pl<!--close .controls-->' . PHP_EOL;
	$output .= '<p class="alert"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . PHP_EOL;
	$output .= '</div><!--close .form-group-->' . PHP_EOL;
	...
	if ( isset( $captcha ) && $captcha === 'true' ) {
		$output .= '<div class="form-group">' . "\r\n";
		$output .= '<img id="captcha" src="' . THEME_URL . 'securimage/securimage_show.php" alt="' . esc_attr__( 'CAPTCHA Image', 'sp-theme' ). '" />' . "\r\n";
		$output .= '<label class="field-label control-label">' . __( 'Enter the characters you see in the image', 'sp-theme' ) . ' <span class="required">*</span></label>' . "\r\n";
		$output .= '<div class="controls">' . "\r\n";
		$output .= '<input type="text" name="captcha_code" size="10" maxlength="6" class="form-control" data-required="required" tabindex="6" /><a href="#" onclick="' . esc_js( 'document.getElementById("captcha").src="' . THEME_URL . 'securimage/securimage_show.php?" + Math.random(); return false' ) . '" class="reset-captcha">[ ' . __( 'Different Image', 'sp-theme' ) . ' ]</a>' . "\r\n";
		$output .= '</div><!--close .controls-->' . "\r\n";
		$output .= '<p class="alert"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";
		$output .= '</div><!--close .form-group-->' . "\r\n";
	}

	$output .= '<p class="alert main"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";	
	$output .= '<div class="form-actions clearfix">' . "\r\n";
	$output .= '<button type="submit" class="btn btn-primary" name="submit" value="submit-register-form"><i class="loader" aria-hidden="true"></i> ' . __( 'Register', 'sp-theme' ) . '</button>' . "\r\n";
	$output .= '</div><!--close .form-actions-->' . "\r\n";

	$output .= wp_nonce_field( 'sp_submit_register_form', '_sp_submit_register_form_nonce', true, false );
	$output .= '<input type="hidden" name="from_email" value="' . str_replace( '@', '*', esc_attr( $from_email ) ) . '" />' . "\r\n";
	$output .= '</form>' . "\r\n";
	$output .= '</div><!--close .sc-register-form-->' . "\r\n";

	// check if users can register
	if ( false === sp_users_can_register() )
		$output = '<div class="sc-register-form"><p>' . __( 'Sorry, new registration is not allowed at this time.', 'sp-theme' ) . '</p></div>' . "\r\n";
...
function sp_login_shortcode( $atts, $content = null ) {
	...
	$output .= '<div class="sc-login-form login ' . esc_attr( $custom_class ) . '">' . "\r\n";
	$output .= '<div class="heading clearfix">' . "\r\n";
	$output .= '<h3 class="title"><span>' . $title . '</span></h3>' . "\r\n";
	$output .= '<div class="meta clearfix">' . "\r\n";
	$output .= '<i class="icon-signin" aria-hidden="true"></i><p class="tagline-wrapper clearfix"><span class="tagline">' . apply_filters( 'sp_sc_login_form_tagline', __( 'Already have an account?', 'sp-theme' ) ) . '</span>' . "\r\n";
	$output .= '<span class="sub-tagline">' . apply_filters( 'sp_sc_login_form_sub_tagline', __( 'Please log in.', 'sp-theme' ) ) . '</span></p>' . "\r\n";
	$output .= '</div><!--close .meta-->' . "\r\n";
	$output .= '</div><!--close .heading-->' . "\r\n";
	$output .= '<form action="#" method="post">' . "\r\n";
	...
	$output .= '<div class="form-group">' . "\r\n";
	$output .= '<label class="field-label control-label">' . __( 'Password', 'sp-theme' ) . '</label>' . "\r\n";
	$output .= '<div class="controls">' . "\r\n";
	$output .= '<input type="password" name="password" class="form-control" data-required="required" tabindex="2" />' . "\r\n";
	$output .= '</div><!--close .controls-->' . "\r\n";
	$output .= '</div><!--close .form-group-->' . "\r\n";	

	$output .= '<div class="form-group">' . "\r\n";
	$output .= '<div class="controls">' . "\r\n";
	$output .= '<label class="field-label control-label"><input type="checkbox" name="rememberme" tabindex="3" /> ' . __( 'Remember Me', 'sp-theme' ) . '</label>' . "\r\n";
	$output .= '</div><!--close .controls-->' . "\r\n";
	$output .= '</div><!--close .form-group-->' . "\r\n";

	$output .= '<p class="alert main"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";	
	$output .= '<div class="form-actions clearfix">' . "\r\n";
	$output .= '<button type="submit" class="btn btn-primary" name="submit" value="submit-login-form"><i class="loader" aria-hidden="true"></i> ' . __( 'Login', 'sp-theme' ) . '</button>' . "\r\n";

	// check show forgot
	if ( isset( $forgot_password ) && $forgot_password === 'true' ) {
		$output .= '<a href="#" class="forgot-password" title="' . esc_attr__( 'Forgot Password', 'sp-theme' ) . '">' . __( 'Forgot Password?', 'sp-theme' ) . '</a>' . "\r\n";
	}
	...
	$output .= '</form>' . "\r\n";
	$output .= '</div><!--close .sc-login-form-->' . "\r\n";

	// show forgot
	if ( isset( $forgot_password ) && $forgot_password === 'true' ) {
		$output .= '<div class="sc-login-form forgot ' . esc_attr( $custom_class ) . '" style="display:none;">' . "\r\n";
		$output .= '<div class="heading clearfix">' . "\r\n";
		$output .= '<h3 class="title"><span>' . apply_filters( 'sp_sc_login_form_forgot_password_title', __( 'Forgot Password', 'sp-theme' ) ) . '</span></h3>' . "\r\n";
		$output .= '</div><!--close .heading-->' . "\r\n";
		$output .= '<form action="#" method="post">' . "\r\n";

		$output .= '<div class="form-group">' . "\r\n";
		$output .= '<label class="field-label control-label">' . __( 'Username or Email', 'sp-theme' ) . ' <span class="required">*</span></label>' . "\r\n";
		$output .= '<div class="controls">' . "\r\n";
		$output .= '<input type="text" name="user_login" class="form-control" data-required="required" tabindex="1" />' . "\r\n";
		$output .= '</div><!--close .controls-->' . "\r\n";
		$output .= '</div><!--close .form-group-->' . "\r\n";

		$output .= '<p class="alert main"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";	
		$output .= '<div class="form-actions clearfix">' . "\r\n";
		$output .= '<button type="submit" class="btn btn-primary" name="submit" value="submit-login-forgot-form"><i class="loader" aria-hidden="true"></i> ' . __( 'Reset Password', 'sp-theme' ) . '</button>' . "\r\n";
		$output .= '<a href="#" class="remember-password" title="' . esc_attr__( 'Remember Password', 'sp-theme' ) . '">' . __( 'I Remember My Login!', 'sp-theme' ) . '</a>' . "\r\n";
		$output .= '</div><!--close .form-actions-->' . "\r\n";
...
function sp_change_password_shortcode( $atts, $content = null ) {
	...
	$output .= '<div class="sc-change-password-form ' . esc_attr( $custom_class ) . '">' . "\r\n";
	$output .= '<div class="heading clearfix">' . "\r\n";
	...
	$output .= '<div class="form-group">' . "\r\n";
	$output .= '<label class="field-label control-label">' . __( 'New Password', 'sp-theme' ) . ' <span class="required">*</span></label>' . "\r\n";
	$output .= '<div class="controls">' . "\r\n";
	$output .= '<input type="password" name="new_password" class="form-control" data-required="required" tabindex="2" />' . "\r\n";
	$output .= '</div><!--close .controls-->' . "\r\n";
	$output .= '<p class="alert"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";
	$output .= '</div><!--close .form-group-->' . "\r\n";	

	$output .= '<div class="form-group">' . "\r\n";
	$output .= '<label class="field-label control-label">' . __( 'Confirm Password', 'sp-theme' ) . ' <span class="required">*</span></label>' . "\r\n";
	$output .= '<div class="controls">' . "\r\n";
	$output .= '<input type="password" name="confirm_password" class="form-control" data-required="required" tabindex="3" />' . "\r\n";
	$output .= '</div><!--close .controls-->' . "\r\n";
	$output .= '<p class="alert"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";
	$output .= '</div><!--close .form-group-->' . "\r\n";	
	...
	$output .= '<p class="alert main"><button class="close" data-dismiss="alert" type="button">&times;</button><span class="msg"></span></p>' . "\r\n";	
	$output .= '<div class="form-actions clearfix">' . "\r\n";
	$output .= '<button type="submit" class="btn btn-primary" name="submit" value="submit-change-password-form"><i class="loader" aria-hidden="true"></i> ' . __( 'Change Password', 'sp-theme' ) . '</button>' . "\r\n";
	...
	if ( $strong_password === 'true' )
		$output .= '<input type="hidden" name="strong_password" value="' . esc_attr( $strong_password ) . '" />' . "\r\n";

	$output .= '</form>' . "\r\n";
	$output .= '</div><!--close .sc-forgot-password-form-->' . "\r\n";
...
function sp_imagelinkbox_shortcode( $atts, $content = null ) {
	...
	$output = '';
	$output .= '<div class="sc-imagelinkbox ' . esc_attr( $custom_class ) . '">' . "\r\n";
	$output .= '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $link_text ) . '">' . "\r\n";
	$output .= '<img src="' . esc_url( $img ) . '" alt="' . esc_attr( $link_text ) . '" />' . "\r\n";

	// if set then show
	if ( isset( $link_text ) && ! empty( $link_text ) )
		$output .= '<h3 class="clearfix">' . esc_attr( $link_text ) . '<i class="icon-angle-right" aria-hidden="true"></i></h3></a>' . "\r\n";

	$output .= '</div><!--close .sc-imagelink-->' . "\r\n";
...
function sp_product_slider_shortcode( $atts, $content = null ) {
	...
	$output .= '<div class="sc-product-slider clearfix ' . esc_attr( $custom_class ) . '">' . "\r\n";
	
	if ( sp_woo_exists() ) 
		$output .= wc_print_notices() . "\r\n";

	// check if we need to display title
	if ( isset( $title ) && ! empty( $title ) )
		$output .= '<h2 class="section-title title-with-line"><span>' . $title . '</span></h2>' . "\r\n";
	...
	$output .= '<input type="hidden" name="slider_options" value="' . esc_attr( json_encode( $options ) ) . '" />' . "\r\n";
	//$output .= '</div><!--close .carousel-container-->' . "\r\n";

	$output .= '</div><!--close .sc-product-slider-->' . "\r\n";
...
function sp_image_link_shortcode( $atts, $content = null ) {
	...
	$output .= '<a href="' . esc_url( $link_url ) . '" title="' . esc_attr( $title ) . '" class="sc-image-link ' . esc_attr( $custom_class ) . '"><img src="' . esc_url( $image_url ) . '" alt="' . esc_attr( $title ) . '" /></a>' . "\r\n";
...
function sp_show_header_shortcode( $atts, $content = null ) {
	...
	$output .= '<header class="header-entry" style="' . esc_attr( $color ) . ' ' . esc_attr( $text_color ) . '">' . "\r\n";
	$output .= '<div class="container">' . "\r\n";

	if ( isset( $tagline_text ) && ! empty( $tagline_text ) )
		$output .= '<h1 class="entry-title">' . $title . '<span class="heading-tagline"> - ' . sanitize_text_field( $tagline_text ) . '</span></h1>' . "\r\n";
	else
		$output .= '<h1 class="entry-title">' . $title . '</h1>' . "\r\n";

	$output .= '</div><!--close .container-->' . "\r\n";
	$output .= '</header><!--close .header-entry-->' . "\r\n";
...
function sp_tooltip_shortcode( $atts, $content = null ) {
	...
	$output = '';
	$output .= '<a href="' . esc_url( $url ) . '" title="' . esc_attr( $tooltip_title ) . '" data-toggle="tooltip" data-placement="' . esc_attr( $position ) . '" class="sp-tooltip">' . $link_name . '</a>' . "\r\n";
...
function sp_bio_card_shortcode( $atts, $content = null ) {
	...
	$output .= '<div class="sc-bio-card clearfix ' . esc_attr( $custom_class ) . '">' . "\r\n";
	$output .= '<img src="' . esc_url( $photo ) . '" alt="' . esc_attr__( 'Photo', 'sp-theme' ) . '" width="150" height="150" />' . "\r\n";
	...
	if ( isset( $email ) && ! empty( $email ) )
		$output .= '<p class="email"><a href="mailto:' . $email . '" title="' . esc_attr__( 'Email', 'sp-theme' ) . ' ' . $name . '">' . $email . '</a></p>' . "\r\n";

	if ( isset( $twitter_name ) && ! empty( $twitter_name ) ) {
		$output .= '<a href="https://twitter.com/' . esc_attr( $twitter_name ) . '" class="twitter-follow-button" data-show-count="false" data-size="normal" data-show-screen-name="true" data-dnt="true">' . __( 'Follow', 'sp-theme' ) . ' @' . $twitter_name . '</a>' . "\r\n";
