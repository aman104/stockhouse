	...
	$module = array(
		...
		'register' => array(
			...
			'options' => array(
				...
				'captcha' => array(
					'requirement' => 'optional',
					'info' => __( 'Enter the title you want to display for this form.  Default is Register', 'sp-theme' ),
					'default' => apply_filters( 'sp_sc_register_form_title', __( 'Register', 'sp-theme' ) ),
					'form_type' => 'text',
					'settings' => array(
...
function sp_shortcodes_input_form( $module = '', $module_settings = '' ) {
	...
	$hidden_name = 'pb[row][][column][][module][][' . $module . '][shortcode_name]';

	$form .= '<input type="hidden" name="' . $hidden_name . '" class="module-value" value="' . esc_attr( $shortcodes[$module]['shortcode_name'] ) . '" />' . "\r\n";
	...
	if ( $shortcodes[$module]['options'] === 'no-option' ) {
		...
		foreach( $shortcodes[$module]['options'] as $k => $v ) {
			...
			$form .= '<div class="form-group ' . esc_attr( $v['requirement'] ) . '">' . "\r\n";
			$form .= '<label><strong>' . ucwords( str_replace( '_', ' ', $k ) ) . ' - <span class="' . $color . '">' . sprintf( __( '%s', 'sp-theme' ), $v['requirement'] ) . '</span></strong><span class="howto">' . $v['info'] . '</span></label>' . "\r\n";
			...
			switch( $v['form_type'] ) {
				...
				case 'select' :
					...
					if ( ! empty( $v['settings'] ) ) {
						$form .= '<select data-set-name="' . esc_attr( $k ) . '" name="' . esc_attr( $name ) . '" class="form-control module-value select2-select">' . "\r\n";
						foreach( $v['settings'] as $setting ) {							
							$form .= '<option value="' . esc_attr( $setting ) . '" ' . selected( isset( $module_settings[$module][$k] ) ? $module_settings[$module][$k] : $v['default'], $setting, false ) . '>' . sprintf( __( '%s', 'sp-theme' ), $setting ) . '</option>' . "\r\n";
						}
				...
				case 'radio' :
					...
					if ( ! empty( $v['settings'] ) ) {
						foreach( $v['settings'] as $setting ) {
							$form .= '<label class="radio">' . sprintf( __( '%s', 'sp-theme' ), ucwords( $setting ) ) . ' <input type="radio" data-set-name="' . esc_attr( $k ) . '" name="' . esc_attr( $name ) . '" class="module-value" value="' . esc_attr( $setting ) . '" ' . checked( isset( $module_settings[$module][$k] ) ? $module_settings[$module][$k] : $v['default'], $setting, false ) . '/></label>' . "\r\n";
						}
					}
					break;
			}
