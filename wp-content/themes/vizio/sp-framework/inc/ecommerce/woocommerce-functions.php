...
// don't show cross_sell products if option is off
if ( sp_get_option( 'show_cross_sell_products', 'is', 'off' ) ) {
...
function sp_woo_add_to_cart_ajax() {
	...
	$interactive_add_to_cart .= '<div class="interactive-modal-content"><p>' . $added_to_cart_message . '</p><div class="button-wrap clearfix"><a href="#" title="' . esc_attr__( 'Continue Shopping', 'sp-theme' ) . '" class="continue-shopping close-modal">' . esc_attr__( 'Continue Shopping', 'sp-theme' ) . '</a><a href="' . esc_url( WC()->cart->get_cart_url() ) . '" title="' . esc_attr__( 'Go to Checkout', 'sp-theme' ) . '" class="checkout">' . esc_attr__( 'Go to Checkout', 'sp-theme' ) . '</a></div></div>';

	echo json_encode( array( 'woo_message' => $woo_message, 'cart' => $cart, 'interactive_add_to_cart' => $interactive_add_to_cart, 'was_added_to_cart' => $was_added_to_cart, 'redirect' => $redirect, 'redirect_url' => $url ) );
	exit;	 
}
...
function sp_woo_product_wishlist_html( $wishlist_ids = array() ) {
	...
	$output .= '<table class="wishlist-table">' . "\r\n";

	$output .= '<thead>' . "\r\n";
	$output .= '<tr>' . "\r\n";
	$output .= '<th class="remove-item"></th>' . "\r\n";
	$output .= '<th class="item-image"></th>' . "\r\n";
	$output .= '<th class="item-name">' . __( 'Item Name', 'sp-theme' ) . '</th>' . "\r\n";
	$output .= '<th class="item-price">' . __( 'Item Price', 'sp-theme' ). '</th>' . "\r\n";
	$output .= '<th class="item-stock">' . __( 'Stock Status', 'sp-theme' ) . '</th>' . "\r\n";
	$output .= '<th class="item-action"></th>' . "\r\n";
	$output .= '</tr>' . "\r\n";
	$output .= '</thead>' . "\r\n";
	$output .= '<tbody>' . "\r\n";
	...
	while( $products->have_posts() ) : $products->the_post();
		...
		$output .= '<tr itemscope itemtype="http://schema.org/Product" class="product">' . "\r\n";

		$output .= '<td><a href="#" title="' . esc_attr__( 'Remove Item', 'sp-theme' ) . '" class="remove-wishlist-item" data-product-id="' . esc_attr( $post->ID ) . '"><i class="icon-remove" aria-hidden="true"></i></a></td>' . "\r\n";
		...
		$output .= '<td class="item-image"><a href="' . get_permalink() . '" title="' . the_title_attribute( 'echo=0' ) . '"><img src="' . esc_attr( $image['url'] ) . '" alt="' . esc_attr( $image['alt'] ) . '" itemprop="image" /></a></td>' . "\r\n";

		$output .= '<td><a href="' . get_permalink() . '" title="' . the_title_attribute( 'echo=0' ) . '">' . get_the_title() . '</a></td>' . "\r\n";

		$output .= '<td class="item-price">' . $product->get_price_html() . '</td>' . "\r\n";
		...
		$output .= '<td class="item-stock"><p class="stock ' . esc_attr( str_replace( ' ', '', strtolower( $stock_status ) ) ) . '">' . $stock_status . '</p></td>' . "\r\n";
		...
		$output .= '<td><form class="cart" method="post" enctype="multipart/form-data">' . $addtocart . '<input type="hidden" name="product_id" value="' . esc_attr( $product->id ) . '" /><input type="hidden" name="product_type" value="simple" /><input type="hidden" name="quantity" value="1" /></form></td>' . "\r\n";
...
function sp_woo_product_compare_html( $compare_ids = array() ) {
	...
	$output .= '<div class="compare-products-container">' . "\r\n";

	$output .= '<ul itemscope itemtype="http://schema.org/Product">' . "\r\n";
	...
	while( $products->have_posts() ) : $products->the_post();
		...
		$output .= '<a href="#" title="' . esc_attr__( 'Remove Item', 'sp-theme' ) . '" class="remove-compare-item" data-product-id="' . esc_attr( $post->ID ) . '"><i class="icon-remove" aria-hidden="true"></i> ' . __( 'Remove Item', 'sp-theme' ) . '</a>' . "\r\n";
		...
		$output .= '<a href="' . get_permalink() . '" title="' . the_title_attribute( 'echo=0' ) . '" class="image-link"><img src="' . esc_attr( $image['url'] ) . '" alt="' . esc_attr( $image['alt'] ) . '" itemprop="image" /></a>' . "\r\n";

		$output .= '<h3><a href="' . get_permalink() . '" title="' . the_title_attribute( 'echo=0' ) . '">' . get_the_title() . '</a></h3>' . "\r\n";

		$output .= '<p class="divider">' . $product->get_price_html() . '</p>' . "\r\n";

		$output .= '<p class="excerpt divider">' . $product->post->post_excerpt . '</p>' . "\r\n";
		...
		$output .= '<p class="stock divider ' . esc_attr( str_replace( ' ', '', strtolower( $stock_status ) ) ) . '">' . $stock_status . '</p>' . "\r\n";
		...
		$output .= '<p class="divider">' . $addtocart . '</p>' . "\r\n";

		$output .= '<input type="hidden" name="product_id" value="' . esc_attr( $post->ID ) . '" />' . "\r\n";

		$output .= '<input type="hidden" name="product_type" value="' . esc_attr( $product->product_type ) . '" />' . "\r\n";
	...
	$output .= '</div><!--close .compare-products-container-->' . "\r\n";
...
function sp_woo_custom_tab_content_display( $key, $tab ) {
	...
	$output .= '</div><!--close .custom-product-tabs-->' . "\r\n";
...
if ( sp_get_option( 'product_view_buttons', 'is', 'on' ) )
...
function sp_woo_display_grid_list_view_buttons() {
	...
	$output .= '<div class="row grid-list-view-buttons">' . "\r\n";
	$output .= '<a href="#" title="' . __( 'List View', 'sp-theme' ) . '" class="list-view-button sp-tooltip" data-toggle="tooltip" data-placement="top"><i class="icon-list-3"></i></a>';
	$output .= '<a href="#" title="' . __( 'Grid View', 'sp-theme' ) . '" class="grid-view-button sp-tooltip" data-toggle="tooltip" data-placement="top"><i class="icon-grid-2"></i></a>' . "\r\n";
...
function sp_woo_product_quickview_ajax() {
	...
	if ( has_post_thumbnail() ) {
		$image_size = sp_get_image_size( $image['url'] );
	} else {
		$image_size = array( 'width' => '' );
	}
	...
	$output .= '<div class="sp-quickview" style="top:2%;">' . "\r\n";

	$output .= '<article itemscope itemtype="http://schema.org/Product" class="product row clearfix">' . "\r\n";
	$output .= '<div class="' . sp_column_css( '', '', '', '5' ) . '">' . "\r\n";
	
	$output .= '<h2 class="product-title" itemprop="name"><a href="' . get_permalink( $product->id ) . '" title="' . esc_attr( get_the_title( $product->id ) ) . '" itemprop="url">' . get_the_title( $product->id ) . '</a></h2>' . "\r\n";

	$output .= '<div class="image-wrap images" data-product-id="' . esc_attr( $product->id ) . '" style="max-width:' . esc_attr( $image_size['width'] ) . 'px;">' . "\r\n";

	$output .= '<a href="' . esc_url( $image_full['url'] ) . '" title="' . esc_attr( get_the_title( $product->id ) ) . '" class="zoom product-image-link" itemprop="image" data-rel="sp-lightbox[' . esc_attr( $product->id ) . ']">' . "\r\n";
	$output .= '<img src="' . esc_attr( $image['url'] ) . '" alt="' . esc_attr( $image['alt'] ) . '" itemprop="image" class="product-image" />' . "\r\n";

	$output .= '<i class="icon-resize-full hover-icon" aria-hidden="true"></i>' . "\r\n";

	// badges
	if ( $product->is_on_sale() && $show_sale_badge === 'on' ) {
		$output .= '<span class="onsale">' . __( 'SALE!', 'sp-theme' ) . '</span>' . "\r\n";
	}
	...
	if ( $show_new_badge === 'on' ) {
		if ( isset( $new_badge_duration ) && ! empty( $new_badge_duration ) ) {
			...
			if ( ( $current_date - $product_date ) < $duration ) {
				$output .= '<span class="new">' . __( 'NEW!', 'sp-theme' ) . '</span>' . "\r\n";
			}
		}
	}
	...
	if ( ! sp_woo_cvo_hide( $product ) ) {
		...
		echo get_post_meta( get_the_ID(), 'Cena jednej sztuki', true, 25 );
		$output .= ob_get_clean();
	}
	...
	$output .= '<a href="' . get_permalink( $product_id ) . '" title="' . esc_attr__( 'Go to product detail page', 'sp-theme' ) . '" itemprop="url" class="product-detail-link">' . apply_filters( 'sp_woo_go_to_product_detail_text', __( 'Product Detail', 'sp-theme' ) ) . '</a>' . "\r\n";

	$output .= '</article><!--close .product-->' . "\r\n";

	$output .= '<button class="close-quickview"><i class="icon-remove-sign" aria-hidden="true"></i></button>' . "\r\n";
	$output .= '</div><!--close .sp-quickview-->' . "\r\n";
...
function sp_woo_email_product_wishlist_ajax() {
			...
			if ( $saved_wishlists && isset( $saved_wishlists[ $wishlist_name ] ) ) {
				foreach( $saved_wishlists[ $wishlist_name ] as $product_id ) {
					...
					if ( sp_woo_exists() ) {
						...
						$link .= apply_filters( 'sp_product_wishlist_email_link', $product_obj->post->post_title . ' - ' . get_permalink( $product_obj->post->ID ), $product_obj->post->post_title, $product_obj->post->ID ) . "\r\n";
					}
				}
			}			
		}
	...
	$email_body = apply_filters( 'sp_product_wishlist_email_opening_text', __( 'Have a look at these awesome products from my wishlist.', 'sp-theme' ) ) . "\r\n\r\n";

	// append the product links
	$email_body .= $link;

	$headers = sprintf( __( '%s', 'sp-theme' ) . ' ' . get_bloginfo( 'name' ) . ' <' . apply_filters( 'sp_product_wishlist_email_from', get_bloginfo( 'admin_email' ) ) . '>', 'From:' ) . "\r\n";
...
function sp_woo_single_product_image_thumbnails_html( $html, $attachment_id, $post_id, $image_class ) {
	...
	if ( $image_swap === 'on' )
		$output .= '<a href="' . esc_url( $image_full['url'] ) . '" title="' . esc_attr( get_the_title( $post_id ) ) . '" class="zoom image-swap" data-attachment-id="' . esc_attr( $attachment_id ) . '" data-product-id="' . esc_attr( $post_id ) . '"><img src="' . esc_url( $image_thumb['url'] ) . '" alt="' . esc_attr( $image_thumb['alt'] ) . '" /><span class="overlay"></span></a>' . "\r\n";
	else
		$output .= '<a href="' . esc_url( $image_full['url'] ) . '" title="' . esc_attr( get_the_title( $post_id ) ) . '" class="' . esc_attr( $image_class ) . '" data-attachment-id="' . esc_attr( $attachment_id ) . '" data-product-id="' . esc_attr( $post_id ) . '"><img src="' . esc_url( $image_thumb['url'] ) . '" alt="' . esc_attr( $image_thumb['alt'] ) . '" /><span class="overlay"></span></a>' . "\r\n";		
...
function sp_woo_product_image_gallery_swap_quickview_ajax() {
	...
	if ( sp_get_option( 'show_product_new_badge' ) === 'on' ) {
		if ( isset( $new_badge_duration ) && ! empty( $new_badge_duration ) ) {
			...
			if ( ( $current_date - $product_date ) < $duration ) {
				$new_badge .= '<span class="new">' . __( 'NEW!', 'sp-theme' ) . '</span>' . "\r\n";
			}
		}
	}
	...
	$output .= '<a href="' . esc_url( $image_full['url'] ) . '" title="' . esc_attr( get_the_title( $product_id ) ) . '" class="zoom product-image-link" itemprop="image"><img src="' . esc_url( $image_thumb['url'] ) . '" alt="' . esc_attr( $image_thumb['alt'] ) . '" /><i class="icon-resize-full hover-icon" aria-hidden="true"></i>' . $sale_flash . $new_badge . '</a>' . "\r\n";
...
function sp_woo_product_rating_html( $product_id = '', $icon_class = 'icon-star' ) {
	...
	$output .= '<div class="product-rating-stars">' . "\r\n";
	...
	for ( $i = 1; $i <= 5; $i++ ) {
		...
		$output .= '<i class="' . esc_attr( $icon_class ) . ' ' . esc_attr( $active ) . '" aria-hidden="true"></i>' . "\r\n";
	}

	$output .= '</div><!--close product-rating-stars-->' . "\r\n";
...
function sp_woo_checkout_additional_info() {
	...
	$output .= '<div class="checkout-additional-info row">' . "\r\n";
	
	$output .= '<div class="' . sp_column_css( '12', '', '', '6' ) . '">' . "\r\n";
	...
	if ( is_array( $links ) && $links ) {
		...
		$output .= '<div class="clearfix links-column ' . sp_column_css( '12', '', '', '6' ) . '">' . "\r\n";
		$output .= '<ul class="clearfix">' . "\r\n";

		foreach( $links['link_name'] as $link ) {
			$output .= '<li class="info-link">' . "\r\n";
			$output .= '<a href="#content-info-' . esc_attr( $i ) . '" title="' . esc_attr( stripslashes( stripslashes( $link ) ) ) . '" class="mfp-link">' . stripslashes( stripslashes( $link ) ) . '</a>' . "\r\n";
			$output .= '<div class="content" id="content-info-' . esc_attr( $i ) . '">' . "\r\n";
			$output .= do_shortcode( stripslashes( stripslashes( wpautop( $links['link_content'][ $i ] ) ) ) ) . "\r\n";
	...
	$output .= '</div><!--close .checkout-additional-info-->' . "\r\n";
...
function sp_woo_checkout_show_review_order_link( $checkout ) {
	...
	$output .= '<p class="goto-review-wrap"><a href="#" class="goto-review button" title="' . esc_attr__( 'Review Order', 'sp-theme' ) . '">' . __( 'Review Order', 'sp-theme' ) . '</a></p>' . "\r\n";
...
function sp_compare_button_html( $product_id ) {
	...
	$output .= apply_filters( 'sp_compare_button_link_html', '<a href="#" class="sp-tooltip compare-button ' . esc_attr( $compare_class ) . '" title="' . esc_attr__( 'Add to Compare', 'sp-theme' ) . '" data-toggle="tooltip" data-placement="top"><i class="icon-cog" aria-hidden="true"></i><span class="text">' . __( 'Compare', 'sp-theme' ) . '</span></a>' ) . "\r\n";
...
function sp_wishlist_button_html( $product_id ) {
	...
	$output .= apply_filters( 'sp_wishlist_button_link_html', '<a href="#" class="sp-tooltip wishlist-button ' . esc_attr( $wishlist_class ) . '" title="' . __( 'Add to Wishlist', 'sp-theme' ) . '" data-toggle="tooltip" data-placement="top"><i class="icon-star" aria-hidden="true"></i><span class="text">' . __( 'Wishlist', 'sp-theme' ) . '</span></a>' ) . "\r\n";
...
function sp_woo_view_order_ajax() {
	...
	$output .= '<div class="order-detail-popup clearfix">' . "\r\n";
	...
	$output .= '</div><!--close .order-detail-popup-->' . "\r\n";
...
function sp_woo_edit_address_ajax() {
	...
	$output .= '<div class="edit-address-popup clearfix">' . "\r\n";
	...
	$output .= '</div><!--close .edit-address-popup-->' . "\r\n";
...
function sp_woo_page_top_widget() {
	...
	if ( sp_woo_exists() ) {
		if ( is_object( $post ) && sp_get_option( 'custom_product_category_widget', 'isset' ) && is_array( sp_get_option( 'custom_product_category_widget' ) ) ) {
			// get the current product's terms
			...
			foreach( $cat_ids as $cat ) {
				...
				if ( is_array( $cat_obj ) ) {
					// checks if the current product category matches the saved settings
