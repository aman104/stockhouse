			...
			<div class="content <?php echo sp_column_css( '', '', '', '9' ); ?>">
				...
				if ( sp_woo_exists() ) {
					...
					if ( isset( $_GET['wishlist'] ) && ! empty( $_GET['wishlist'] ) ) {
						...
						echo '<h3 class="wishlist-name">' . __( 'Wishlist Name:', 'sp-theme' ) . ' ' . stripslashes( $wishlist_name ) . '</h3>' . "\r\n";
					...
					if ( isset( $wishlist_ids ) && ! empty( $wishlist_ids ) )
						...
						echo '<p>' . __( 'There are no items in your wishlist.', 'sp-theme' ) . '</p>' . "\r\n";
				}
				...
				<div class="wishlist-action row">
					...
					if ( ( sp_users_can_register() || is_user_logged_in() ) && isset( $wishlist_ids ) && ! empty( $wishlist_ids ) ) {
						...
						if ( is_user_logged_in() ) {
							...
							$output .= '<p>' . "\r\n";
							$output .= '<label>' . __( 'Save Wishlist', 'sp-theme' ) . '</label>' . "\r\n";
							$output .= '<select name="product_wishlist_save_type" class="select2-select">' . "\r\n";
							$output .= '<option value="0">' . __( '--Please Select--', 'sp-theme' ) . '</option>' . "\r\n";
							$output .= '<option value="new">' . __( 'Save New Wishlist', 'sp-theme' ) . '</option>' . "\r\n";

							// if there are existing saved wishlists
							if ( $saved_wishlists ) 
								$output .= '<option value="existing">' . __( 'Save to Existing Wishlist', 'sp-theme' ) . '</option>' . "\r\n";

							$output .= '</select>' . "\r\n";
							$output .= '</p>' . "\r\n";

							$output .= '<p class="new">';
							$output .= '<label>' . __( 'New Wishlist Name', 'sp-theme' ) . '</label>' . "\r\n";
							$output .= '<input type="text" name="wishlist_name" value="" class="form-control" /><br />' . "\r\n";
							$output .= '<a href="" title="' . __( 'Save Wishlist', 'sp-theme' ) . '" class="save-wishlist button" data-wishlist-type="new">' . __( 'Save Wishlist', 'sp-theme' ) . '</a>' . "\r\n";
							$output .= '</p>';

							if ( isset( $saved_wishlists ) && $saved_wishlists ) {
								$output .= '<p class="existing">';
								$output .= '<label>' . __( 'Save to Existing Wishlist', 'sp-theme' ) . '</label>' . "\r\n";
								$output .= '<select name="existing_wishlist" class="select2-select">' . "\r\n";
								...
								$output .= '</select><br /><br />' . "\r\n";
								$output .= '<a href="" title="' . __( 'Save Wishlist', 'sp-theme' ) . '" class="save-wishlist button" data-wishlist-type="existing">' . __( 'Save Wishlist', 'sp-theme' ) . '</a>' . "\r\n";
								$output .= '</p>';						
							}
						} else {
				...
				<div class="email-product-wishlist-container row">
